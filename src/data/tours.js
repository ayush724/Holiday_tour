export const featuredTours = [
  {
    id: "golden-triangle",
    title: "Golden Triangle Tour",
    description: "Explore the cultural heritage of Delhi, Agra, and Jaipur",
    duration: "6 Days / 5 Nights",
    // price: ,
    image:
      "https://images.unsplash.com/photo-1524492412937-b28074a5d7da?q=80&w=1200",
    rating: 4.8,
    locations: ["Delhi", "Agra", "Jaipur"],
    category: "Cultural",
    featured: true,
    highlights: [
      "Visit the iconic Taj Mahal at sunrise",
      "Explore the majestic Amber Fort in Jaipur",
      "Experience the vibrant bazaars of Old Delhi",
      "Luxury accommodations throughout the journey",
      "Private air-conditioned vehicle with experienced driver",
    ],
    itinerary: [
      {
        day: 1,
        title: "Arrival in Delhi",
        description:
          "Arrive at Delhi International Airport. Meet and greet with our representative. Transfer to hotel. Evening at leisure.",
        activities: ["Airport pickup", "Hotel check-in", "Welcome dinner"],
      },
      {
        day: 2,
        title: "Delhi City Tour",
        description:
          "Full day sightseeing of Old and New Delhi visiting major attractions.",
        activities: [
          "Red Fort",
          "Jama Masjid",
          "Chandni Chowk",
          "Qutub Minar",
          "India Gate",
        ],
      },
      {
        day: 3,
        title: "Delhi to Agra",
        description:
          "Morning drive to Agra. Afternoon visit to Taj Mahal and Agra Fort.",
        activities: [
          "Morning drive to Agra",
          "Taj Mahal visit",
          "Agra Fort exploration",
        ],
      },
      {
        day: 4,
        title: "Agra to Jaipur",
        description:
          "Drive to Jaipur via Fatehpur Sikri. Check-in at hotel in Jaipur.",
        activities: [
          "Visit Fatehpur Sikri",
          "Lunch at heritage restaurant",
          "Evening at leisure in Jaipur",
        ],
      },
      {
        day: 5,
        title: "Jaipur Sightseeing",
        description:
          "Full day Jaipur tour including Amber Fort, City Palace, and more.",
        activities: [
          "Amber Fort with elephant ride",
          "City Palace",
          "Jantar Mantar",
          "Hawa Mahal",
          "Local market visit",
        ],
      },
      {
        day: 6,
        title: "Departure",
        description: "Transfer to Jaipur airport for onward journey.",
        activities: ["Breakfast at hotel", "Airport transfer"],
      },
    ],
  },

  {
    id: "kerala-backwaters",
    title: "Kerala Backwaters",
    description:
      "Relax and rejuvenate in God's own country with stunning backwaters",
    duration: "5 Days / 4 Nights",
    // price: 599,
    image:
      "https://images.unsplash.com/photo-1602216056096-3b40cc0c9944?q=80&w=1032&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    rating: 4.9,
    locations: ["Kochi", "Munnar", "Alleppey"],
    category: "adventure",
    featured: true,
    highlights: [
      "Overnight stay in traditional houseboat",
      "Spice plantation tour in Munnar",
      "Kathakali dance performance in Kochi",
      "Ayurvedic massage treatment",
      "Fresh seafood cuisine experiences",
    ],
    itinerary: [
      {
        day: 1,
        title: "Arrival in Kochi",
        description:
          "Arrive at Kochi International Airport. Transfer to hotel. Evening Kathakali dance performance.",
        activities: [
          "Airport pickup",
          "Hotel check-in",
          "Kathakali dance show",
        ],
      },
      {
        day: 2,
        title: "Kochi City Tour",
        description:
          "Full day sightseeing in Fort Kochi area including Chinese fishing nets, St. Francis Church, and Mattancherry Palace.",
        activities: [
          "Visit Chinese fishing nets",
          "Explore Dutch Palace",
          "Walk through Jew Town",
          "Shopping at local markets",
        ],
      },
      {
        day: 3,
        title: "Kochi to Munnar",
        description:
          "Morning drive to Munnar. Afternoon visit to tea plantations and Eravikulam National Park.",
        activities: [
          "Scenic drive to Munnar",
          "Tea plantation visit",
          "Wildlife spotting at national park",
        ],
      },
      {
        day: 4,
        title: "Munnar to Alleppey",
        description:
          "Travel to Alleppey. Board a traditional houseboat for an overnight stay on the backwaters.",
        activities: [
          "Transfer to Alleppey",
          "Houseboat check-in",
          "Backwater cruise",
          "Traditional Kerala dinner onboard",
        ],
      },
      {
        day: 5,
        title: "Departure",
        description:
          "Disembark from houseboat. Transfer to Kochi airport for departure.",
        activities: [
          "Morning backwaters sunrise",
          "Breakfast on houseboat",
          "Airport transfer",
        ],
      },
    ],
  },
  {
    id: "rajasthan-heritage",
    title: "Rajasthan Heritage",
    description:
      "Discover the royal heritage and majestic palaces of Rajasthan",
    duration: "8 Days / 7 Nights",
    // price: 899,
    image:
      "https://images.unsplash.com/photo-1599661046289-e31897846e41?q=80&w=1200",
    rating: 4.7,
    locations: ["Jaipur", "Udaipur", "Jodhpur", "Jaisalmer"],
    category: "Cultural",
    featured: true,
    highlights: [
      "Camel safari in Thar Desert",
      "Boat ride on Lake Pichola",
      "Stay in heritage palaces",
      "Traditional Rajasthani folk dance performances",
      "Desert camping experience",
    ],
    itinerary: [
      {
        day: 1,
        title: "Arrival in Jaipur",
        description:
          "Arrive at Jaipur Airport. Transfer to heritage hotel. Evening at leisure to explore the pink city.",
        activities: [
          "Airport pickup",
          "Heritage hotel check-in",
          "Welcome dinner with cultural program",
        ],
      },
      {
        day: 2,
        title: "Jaipur Sightseeing",
        description:
          "Full day tour of Jaipur including Amber Fort, City Palace, and Hawa Mahal.",
        activities: [
          "Elephant ride at Amber Fort",
          "Jal Mahal visit",
          "Shopping for textiles and handicrafts",
        ],
      },
      {
        day: 3,
        title: "Jaipur to Jodhpur",
        description:
          "Drive to Jodhpur with stop at Ajmer Sharif Dargah. Evening arrival in the Blue City.",
        activities: [
          "Visit to Ajmer Sharif",
          "Scenic drive through Aravalli hills",
          "Dinner at rooftop restaurant",
        ],
      },
      {
        day: 4,
        title: "Jodhpur Exploration",
        description:
          "Visit Mehrangarh Fort, Jaswant Thada, and explore the blue-painted old city.",
        activities: [
          "Mehrangarh Fort tour",
          "Old city walking tour",
          "Visit to local spice market",
        ],
      },
      {
        day: 5,
        title: "Jodhpur to Jaisalmer",
        description:
          "Travel to Jaisalmer, the Golden City. Evening visit to Gadisar Lake.",
        activities: [
          "Desert drive to Jaisalmer",
          "Sunset at Gadisar Lake",
          "Rajasthani folk music performance",
        ],
      },
      {
        day: 6,
        title: "Jaisalmer and Desert Safari",
        description:
          "Morning tour of Jaisalmer Fort and havelis. Afternoon camel safari and desert camping.",
        activities: [
          "Jaisalmer Fort tour",
          "Patwon Ki Haveli visit",
          "Camel safari",
          "Overnight desert camping",
        ],
      },
      {
        day: 7,
        title: "Jaisalmer to Udaipur",
        description:
          "Long drive to Udaipur, the City of Lakes. Evening at leisure.",
        activities: [
          "Scenic drive through Rajasthan",
          "Rest stops at local villages",
          "Evening lake view dinner",
        ],
      },
      {
        day: 8,
        title: "Departure from Udaipur",
        description:
          "Morning boat ride on Lake Pichola. Visit to City Palace. Transfer to airport for departure.",
        activities: [
          "Lake Pichola boat ride",
          "City Palace tour",
          "Airport transfer",
        ],
      },
    ],
  },
  {
    id: "varanasi-spiritual",
    title: "Varanasi Spiritual Journey",
    description:
      "Experience the spiritual essence of India's oldest living city",
    duration: "4 Days / 3 Nights",
    // price: 499,
    image:
      "https://images.unsplash.com/photo-1706186839147-0d708602587b?q=80&w=870&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    rating: 4.6,
    locations: ["Varanasi", "Sarnath"],
    category: "Spiritual",
    featured: true,
    highlights: [
      "Witness the sacred Ganga Aarti ceremony",
      "Sunrise boat ride on the River Ganges",
      "Visit to Buddhist pilgrimage site Sarnath",
      "Explore ancient temples and narrow lanes",
      "Experience traditional music and meditation",
    ],
    itinerary: [
      {
        day: 1,
        title: "Arrival in Varanasi",
        description:
          "Arrive at Varanasi Airport. Transfer to hotel near the ghats. Evening visit to witness the spectacular Ganga Aarti ceremony at Dashashwamedh Ghat.",
        activities: [
          "Airport pickup",
          "Hotel check-in",
          "Ganga Aarti ceremony",
        ],
      },
      {
        day: 2,
        title: "Varanasi Ghats and Temples",
        description:
          "Early morning boat ride on the Ganges to witness the sunrise. After breakfast, visit important temples including Kashi Vishwanath and Sankat Mochan.",
        activities: [
          "Sunrise boat ride",
          "Temple tours",
          "Walking tour of old city",
          "Evening music performance",
        ],
      },
      {
        day: 3,
        title: "Excursion to Sarnath",
        description:
          "Day trip to Sarnath where Buddha gave his first sermon. Visit the Dhamek Stupa, Archaeological Museum, and the Bodhi tree. Evening meditation session by the Ganges.",
        activities: [
          "Sarnath Buddhist sites",
          "Archaeological Museum",
          "Meditation session",
          "Farewell dinner",
        ],
      },
      {
        day: 4,
        title: "Departure",
        description:
          "Optional early morning visit to the ghats. After breakfast, transfer to airport for departure.",
        activities: [
          "Optional morning ghats visit",
          "Breakfast at hotel",
          "Airport transfer",
        ],
      },
    ],
  },
];
export const CharDhamYatra = [
  {
    id: "char-dham-yatra",
    title: "Chardham Yatra",
    description:
      "Chardham Yatra is a beautiful and religious trip of the Himalayas. Chardham Yatra 2025 starts from late April. The tour includes spiritual, Himalayan scenery, pilgrim experiences and includes Ganga Aarti, Himalayan views and stays in Uttarakhand. Different packages (Luxury, Deluxe, Budget) are available. Advance booking is a must.",
    duration: "12 Days / 11 Nights",
    // price: "",
    image: "https://www.allinharidwar.com/wp-content/uploads/2018/04/Rishikesh-to-Chardham-Yatra.jpg", // images present, but no single “featured” image URL was extracted
    rating: "", // not provided
    locations: [
      "Delhi",
      "Haridwar",
      "Barkot",
      "Yamunotri",
      "Uttarkashi",
      "Gangotri",
      "Rudraprayag",
      "Kedarnath",
      "Joshimath",
      "Badrinath",
      "Rishikesh",
    ],
    category: "Spiritual",
    featured: false,
    highlights: [
      "Ganga Aarti of Haridwar & Rishikesh",
      "Introduction to Indian spirituality through pilgrimage",
      "Panoramic views of the Himalayas",
      "Special features: multiple types of package (Luxury, Deluxe, Budget) with different standards of hotels or camps",
    ],
    itinerary: [
      {
        day: 1,
        title: "Delhi to Haridwar",
        description:
          "Arrive in Delhi and drive to Haridwar. In the evening, attend the sacred Ganga Aarti at Har-ki-Pauri.",
        activities: [
          "Drive from Delhi to Haridwar",
          "Evening Ganga Aarti at Har-ki-Pauri",
        ],
      },
      {
        day: 2,
        title: "Haridwar to Barkot",
        description:
          "Travel from Haridwar to Barkot via Dehradun and Mussoorie. Scenic drive through hills and valleys.",
        activities: [
          "Drive through Dehradun",
          "Stopover at Mussoorie",
          "Arrival at Barkot",
        ],
      },
      {
        day: 3,
        title: "Barkot to Yamunotri and back",
        description:
          "Early morning drive to Janki Chatti, then trek/horse/doli ride to Yamunotri. Visit Yamunotri temple before returning to Barkot.",
        activities: [
          "Drive to Janki Chatti",
          "Trek/horse/doli to Yamunotri",
          "Yamunotri Darshan",
          "Return to Barkot",
        ],
      },
      {
        day: 4,
        title: "Barkot to Uttarkashi",
        description:
          "Drive to Uttarkashi and visit the famous Vishwanath Temple in the evening.",
        activities: ["Drive to Uttarkashi", "Visit Vishwanath Temple"],
      },
      {
        day: 5,
        title: "Uttarkashi to Gangotri and back",
        description:
          "Full-day excursion to Gangotri, one of the Char Dhams. Return to Uttarkashi for overnight stay.",
        activities: [
          "Scenic drive to Gangotri",
          "Gangotri Temple Darshan",
          "Return to Uttarkashi",
        ],
      },
      {
        day: 6,
        title: "Uttarkashi to Rudraprayag",
        description:
          "Drive from Uttarkashi to Rudraprayag via the Tehri Dam route.",
        activities: ["Drive through Tehri Dam", "Arrival at Rudraprayag"],
      },
      {
        day: 7,
        title: "Rudraprayag to Kedarnath",
        description:
          "Travel to Gaurikund and begin trek to Kedarnath. Stay overnight near the Kedarnath Temple.",
        activities: [
          "Drive to Gaurikund",
          "Trek to Kedarnath",
          "Darshan at Kedarnath Temple",
          "Overnight stay at Kedarnath",
        ],
      },
      {
        day: 8,
        title: "Kedarnath to Rudraprayag",
        description:
          "Return from Kedarnath trek to Gaurikund and drive back to Rudraprayag via Guptkashi.",
        activities: [
          "Trek back from Kedarnath",
          "Drive via Guptkashi",
          "Arrival at Rudraprayag",
        ],
      },
      {
        day: 9,
        title: "Rudraprayag to Badrinath",
        description:
          "Drive to Badrinath via Joshimath. Evening darshan at the Badrinath Temple.",
        activities: [
          "Scenic drive to Joshimath",
          "Continue to Badrinath",
          "Evening Badrinath Temple Darshan",
        ],
      },
      {
        day: 10,
        title: "Badrinath to Joshimath",
        description:
          "Morning darshan at Badrinath, then drive to Joshimath. Visit nearby attractions like Mana village and Vyas Gufa.",
        activities: [
          "Morning Badrinath Darshan",
          "Visit Mana Village",
          "Visit Vyas Gufa",
          "Drive to Joshimath",
        ],
      },
      {
        day: 11,
        title: "Joshimath to Rishikesh",
        description:
          "Drive to Rishikesh. Visit Narsingh Temple and Shankaracharya Math. Evening free at leisure.",
        activities: [
          "Drive to Rishikesh",
          "Visit Narsingh Temple",
          "Visit Shankaracharya Math",
        ],
      },
      {
        day: 12,
        title: "Rishikesh to Delhi (via Haridwar)",
        description:
          "Drive back to Delhi via Haridwar. Departure for onward journey.",
        activities: [
          "Breakfast at hotel",
          "Drive to Haridwar",
          "Continue to Delhi",
          "Departure",
        ],
      },
    ],
    itinerary: [
      {
        day: 1,
        title: "Delhi to Haridwar",
        description:
          "Arrive in Delhi and drive to Haridwar. In the evening, attend the Ganga Ceremony at Har Ki Pauri.",
        activities: [
          "Drive from Delhi to Haridwar",
          "Evening Ganga Ceremony at Har Ki Pauri",
        ],
      },
      {
        day: 2,
        title: "Haridwar to Barkot",
        description:
          "Drive from Haridwar to Barkot via Dehradun and Mussoorie. Stop at Satya Narayan Temple, Prakasheswar Temple, and Kempty Falls.",
        activities: [
          "Drive through Dehradun and Mussoorie",
          "Visit Satya Narayan Temple",
          "Visit Prakasheswar Temple",
          "Visit Kempty Falls",
          "Arrive at Barkot",
        ],
      },
      {
        day: 3,
        title: "Barkot to Yamunotri and back",
        description:
          "Travel to Janki Chatti and trek to Yamunotri Temple. Return to Barkot in the evening.",
        activities: [
          "Drive to Janki Chatti",
          "Trek to Yamunotri Temple",
          "Yamunotri Darshan",
          "Return to Barkot",
        ],
      },
      {
        day: 4,
        title: "Barkot to Haridwar",
        description:
          "Drive back to Haridwar via Devprayag, visiting Vasistha Gufa, Laxman Jhula, and Ram Jhula along the way.",
        activities: [
          "Drive via Devprayag",
          "Visit Vasistha Gufa",
          "Visit Laxman Jhula",
          "Visit Ram Jhula",
          "Arrive at Haridwar",
        ],
      },
      {
        day: 5,
        title: "Haridwar to Delhi",
        description:
          "Drive back to Delhi with visits en route to Daksh Temple, Shri Yantra Temple, and Maya Devi Temple.",
        activities: [
          "Visit Daksh Temple",
          "Visit Shri Yantra Temple",
          "Visit Maya Devi Temple",
          "Drive to Delhi",
          "Departure",
        ],
      },
    ],

    contact: {
      phone: ["+91-9412072802", "+91-8630090671", "+91-9456332153"],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
  },

  {
    id: "yamunotri-dham-yatra",
    title: "Yamunotri Dham Yatra",
    description:
      "The source of river Yamuna, Yamunotri, is the westernmost shrine in Chardham. Situated on the top of Bandar Poonch Parvat the temple is devoted to goddess Yamuna. Although the actual source of river Yamuna is the Champasar Glacier or the Yamunotri Glacier is located at a distance of 1 km from the temple on Kalind Mountain, but is not accessible so the temple itself is worshiped as the source of Yamuna. Yamunotri is reached after a 7 km trek from Janki Chatti. The temple was built by Maharaja Pratap Shah of Tehri Garhwal on the banks of the river and the deity that resides here is made of black marble. The river Yamuna has been elevated to the status of a divine mother by the Hindu mythology for its role in nurturing the civilization in the Gangetic plain. There are hot water springs near the temple forming Kunds among which Surya Kund is the most important. Near this kund is a rock known as Divya Shila. The Divya Shila is worshiped before the deity. Devotees prepare potato and rice by tying them in a muslin cloth dipping them in the hot water springs and offer them to the deity. The rice is taken back as Prasad. The temple remains closed during the winters as the area is prone to heavy snowfall.",
    duration: "5 Days / 4 Nights",
    // price: "", // as listed price on the page :contentReference[oaicite:0]{index=0}
    image: "https://www.allinharidwar.com/library/timthumb.php?src=/wp-content/uploads/2018/02/Yamunotri-Dham-Yatra-1.jpg&w=628&h=380&zc=0&q=80", // multiple images are present but no single “featured” image URL extracted :contentReference[oaicite:1]{index=1}
    rating: "", // not given on the page :contentReference[oaicite:2]{index=2}
    locations: ["Delhi", "Haridwar", "Barkot", "Janki Chatti", "Yamunotri"],
    category: "Spiritual",
    featured: false,
    highlights: [
      "Visit Yamunotri temple, the westernmost shrine of Chardham",
      "Trek 7 km from Janki Chatti to Yamunotri temple",
      "See hot water springs (Surya Kund), Divya Shila rock, ritual of “prasād” cooked in springs",
      "Includes Ganga ceremony in Haridwar, scenic drives, and stops at places like Satya Narayan Temple, Prakasheswar Temple, Kempty Falls",
    ],
    itinerary: [
      {
        day: 1,
        title: "Delhi to Haridwar",
        description:
          "Arrive in Delhi and drive to Haridwar. Attend the evening Ganga Ceremony at Har Ki Pauri.",
        activities: [
          "Drive from Delhi to Haridwar",
          "Evening Ganga Ceremony at Har Ki Pauri",
        ],
      },
      {
        day: 2,
        title: "Haridwar to Rudraprayag / Guptkashi",
        description:
          "Travel from Haridwar to Rudraprayag / Guptkashi via Devprayag, visiting Satyanarayan Temple along the way.",
        activities: [
          "Drive from Haridwar to Rudraprayag / Guptkashi via Devprayag",
          "Visit Satyanarayan Temple en route",
        ],
      },
      {
        day: 3,
        title: "Rudraprayag / Sitapur to Kedarnath and back",
        description:
          "Travel to Gaurikund, then trek or use horse/pony/dolly/helicopter to reach Kedarnath. Return to Rudraprayag / Sitapur in the evening.",
        activities: [
          "Drive to Gaurikund",
          "Trek / Horse / Pony / Dolly / Helicopter to Kedarnath",
          "Visit Kedarnath Temple",
          "Return to Rudraprayag / Sitapur",
        ],
      },
      {
        day: 4,
        title: "Rudraprayag / Sitapur to Haridwar",
        description:
          "Drive back to Haridwar via Rishikesh and Devprayag, enjoying scenic views along the way.",
        activities: [
          "Drive via Rishikesh & Devprayag",
          "Arrive in Haridwar",
          "Leisure time",
        ],
      },
      {
        day: 5,
        title: "Haridwar to Delhi",
        description:
          "Drive back to Delhi, visiting Daksh Temple, Shri Yantra Temple, and Maya Devi Temple en route. Depart from Delhi.",
        activities: [
          "Drive from Haridwar to Delhi",
          "Visit Daksh Temple, Shri Yantra Temple, Maya Devi Temple en route",
          "Departure from Delhi",
        ],
      },
    ],
    contact: {
      phone: ["+91-9412072802", "+91-9456332153", "8630090671"],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
  },
  {
    id: "gangotri-dham-yatra",
    title: "Gangotri Dham Yatra",
    description:
      "The holy place overlooks the thundering river Bhagirathi which flows between the rugged mountains; Gangotri is the spiritual source of India’s holiest river Ganga. Here Ganga is known as Bhagirathi. The actual source is the ice cave of Gaumukh. It is believed that bathing in her waters brings liberation from all sins committed in the present and past birth. According to legend, after King Bhagirath’s prayers, goddess Ganga agreed to descend from heaven, Lord Shiva caught her in his locks (to prevent the destruction of Earth), and released her drops onto the earth to cleanse it. ",
    duration: "4 Nights / 5 Days",
    // price: "",
    image: "https://images.unsplash.com/photo-1696465889052-6c8921536920?q=80&w=1032&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D", // there are photos on the page but no single “featured image” URL was clearly extracted
    rating: "", // one review is indicated but no star rating visible
    locations: ["Delhi", "Haridwar", "Uttarkashi", "Gangotri"],
    category: "Spiritual",
    featured: false,
    highlights: [
      "Holy bathing at the source of the river Ganga (Bhagirathi) in Gangotri",
      "Visit Uttarkashi valley and its scenic surroundings",
      "Ganga Aarti at Har-Ki-Pauri in Haridwar",
      "Scenic travel through mountainous terrain with river views",
    ],
    itinerary: [
      {
        day: 1,
        title: "Delhi to Haridwar",
        description:
          "Arrive in Delhi and drive to Haridwar. Attend the Ganga Aarti at Har-Ki-Pauri in the evening.",
        activities: [
          "Drive from Delhi to Haridwar",
          "Evening Ganga Aarti at Har-Ki-Pauri",
        ],
      },
      {
        day: 2,
        title: "Haridwar to Uttarkashi",
        description:
          "Drive from Haridwar to Uttarkashi via a scenic route through the mountains.",
        activities: [
          "Drive from Haridwar to Uttarkashi",
          "Enjoy scenic mountain views en route",
        ],
      },
      {
        day: 3,
        title: "Uttarkashi to Gangotri and back",
        description:
          "Travel from Uttarkashi to Gangotri, perform holy dip and pooja at the Gangotri Temple, then return to Uttarkashi.",
        activities: [
          "Drive to Gangotri",
          "Holy dip at Gangotri",
          "Pooja at Gangotri Temple",
          "Return to Uttarkashi",
        ],
      },
      {
        day: 4,
        title: "Uttarkashi to Haridwar",
        description:
          "Drive back to Haridwar with a lunch stop at Chamba. Leisure time upon arrival.",
        activities: [
          "Drive via Chamba",
          "Lunch at Chamba",
          "Arrive in Haridwar",
          "Leisure time",
        ],
      },
      {
        day: 5,
        title: "Haridwar to Delhi",
        description:
          "Drive back to Delhi and transfer to the railway station or airport for departure.",
        activities: [
          "Drive from Haridwar to Delhi",
          "Transfer to railway station/airport",
          "Departure",
        ],
      },
    ],

    contact: {
      phone: ["+91-9412072802", "+91-9456332153", "8630090671"],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
  },
  {
    id: "kedarnath-dham-yatra",
    title: "Kedarnath Dham Yatra",
    description:
      "One of the twelve Jyotirlingas dedicated to the Sadashiva form of Lord Shiva and the third shrine in Chardham, Kedarnath is a magnificent temple situated at a height of 3581m. In the temple a huge statue of Nandi stands. There are a number of hot water springs called as Kunds in Kedarnath. Among which Shiv Kund is the most important and Rudhir Kund is famous for its red colored water. The whole valley of Kedarnath is surrounded by snow-covered peaks from three sides with Madakini and Gandhi Glaciers on the fourth side only a few kilometers away. Among the Chardham, Kedarnath is situated at the highest altitude. It is reached after a 14 km trek from Gaurikund. It is believed that Kedarnath shrine was built by the Pandavas after the battle of Mahabharat. Kedarnath also is the main shrine among the Panch Kedar devoted to Lord Shiva. The other four are Tungnath, Rudranath, Mad Maheshwar, Kalpeshwar. These temples are worshipped by Hindu devotees during their Chardham pilgrimage.",
    duration: "5 Days / 4 Nights",
    // price: "", // price not clearly given
    image: "https://images.unsplash.com/photo-1712733900711-d0b929d0d7cc?q=80&w=377&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D", // image(s) present, but no single “featured” image URL extracted
    rating: "1 Review", // there is “(1 Review)” shown on page :contentReference[oaicite:0]{index=0}
    locations: [
      "Delhi",
      "Haridwar",
      "Rudraprayag / Guptkashi",
      "Gaurikund",
      "Kedarnath",
    ],
    category: "Spiritual",
    featured: false,
    highlights: [
      "Visit Kedarnath Temple, one of the 12 Jyotirlingas and part of Chardham pilgrimage",
      "Trek 14 km from Gaurikund (trek / horse / pony / dolly; option of helicopter service available)",
      "Scenic views: Gonded peaks, Madakini & Gandhi Glaciers around the valley",
      "Ganga Ceremony at Har Ki Pauri in Haridwar, visit Satyanarayan Temple, Devprayag, Guptkashi",
      "Surrounded by snow-covered mountains; high altitude pilgrimage experience",
    ],
    itinerary: [
      {
        day: 1,
        title: "Delhi to Haridwar",
        description:
          "Arrive in Delhi and drive to Haridwar. Attend the evening Ganga Ceremony at Har Ki Pauri.",
        activities: [
          "Drive from Delhi to Haridwar",
          "Evening Ganga Ceremony at Har Ki Pauri",
        ],
      },
      {
        day: 2,
        title: "Haridwar to Rudraprayag / Guptkashi",
        description:
          "Travel from Haridwar to Rudraprayag / Guptkashi via Devprayag, visiting Satyanarayan Temple along the way.",
        activities: [
          "Drive from Haridwar to Rudraprayag / Guptkashi via Devprayag",
          "Visit Satyanarayan Temple en route",
        ],
      },
      {
        day: 3,
        title: "Rudraprayag / Sitapur to Kedarnath and back",
        description:
          "Travel to Gaurikund, then trek or use horse/pony/dolly/helicopter to reach Kedarnath. Return to Rudraprayag / Sitapur in the evening.",
        activities: [
          "Drive to Gaurikund",
          "Trek / Horse / Pony / Dolly / Helicopter to Kedarnath",
          "Visit Kedarnath Temple",
          "Return to Rudraprayag / Sitapur",
        ],
      },
      {
        day: 4,
        title: "Rudraprayag / Sitapur to Haridwar",
        description:
          "Drive back to Haridwar via Rishikesh and Devprayag, enjoying scenic views along the way.",
        activities: [
          "Drive via Rishikesh & Devprayag",
          "Arrive in Haridwar",
          "Leisure time",
        ],
      },
      {
        day: 5,
        title: "Haridwar to Delhi",
        description:
          "Drive back to Delhi, visiting Daksh Temple, Shri Yantra Temple, and Maya Devi Temple en route. Depart from Delhi.",
        activities: [
          "Drive from Haridwar to Delhi",
          "Visit Daksh Temple, Shri Yantra Temple, Maya Devi Temple en route",
          "Departure from Delhi",
        ],
      },
    ],

    contact: {
      phone: ["+91-9412072802", "+91-9456332153", "8630090671"],
      email: ["indiaholidayhome@gmail.com", "amitbajaj13@hotmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
  },
  {
    id: "badrinath-dham-yatra",
    title: "Badrinath Dham Yatra",
    description:
      "The Abode Of Lord Vishnu : – Badrinath Temple situated in the twin mountain ranges of Nar and Narayan is the holiest of the four main shrines, Temple Darshan along the left bank river Alaknanda with the view of splendid Neelkanth mountain and visiting Mana Village are the main attractions, Badrinath is an important destination for every devout Hindu. Once the spot was carpeted with ‘badris’ or wild berries and hence was famous as ‘Badri Van’. Badrinath is considered the holiest of the four important shrines in Garhwal. The town is at an altitude of 3,133 m … The present temple was built about two centuries ago by Garhwal Kings … The temple has three parts – Garbha Griha (sanctum sanctorum), Darshan Mandap, Shobha Mandap … Haridwar is the gateway; pilgrims take the holy Ganga dip before starting the Yatra …",
    duration: "6 Days / 5 Nights",
    // price: "",
    image: "https://images.unsplash.com/photo-1601821139990-9fc929db79ce?q=80&w=870&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D", // images are shown on page but no single “featured” image URL clearly extracted
    rating: "2 Reviews",
    locations: [
      "Delhi",
      "Haridwar",
      "Rudraprayag",
      "Badrinath",
      "Joshimath",
      "Rishikesh",
      "Uttarakhand",
    ],
    category: "Spiritual",
    featured: false,
    highlights: [
      "Darshan of Badrinath Temple at 3,133 m in the Garhwal Himalayas",
      "View of Neelkanth mountain, visit Mana Village",
      "Temple architecture and sanctum structures (Garbha Griha, Darshan Mandap, Shobha Mandap)",
      "Holy Ganga dip in Haridwar before starting yatra; scenic drives through Rudraprayag & Joshimath",
    ],
    itinerary: [
      {
        day: 1,
        title: "Delhi to Haridwar",
        description:
          "Drive from Delhi to Haridwar and attend the evening Ganga Aarti at Har-ki-Pauri.",
        activities: [
          "Drive from Delhi to Haridwar",
          "Evening Ganga Aarti at Har-ki-Pauri",
        ],
      },
      {
        day: 2,
        title: "Haridwar to Rudraprayag",
        description:
          "Travel from Haridwar to Rudraprayag via Deoprayag, enjoying scenic stops along the way.",
        activities: [
          "Drive from Haridwar to Rudraprayag via Deoprayag",
          "Scenic stops en route",
        ],
      },
      {
        day: 3,
        title: "Rudraprayag to Badrinath",
        description:
          "Drive to Badrinath via Joshimath and visit Badrinath Temple in the evening.",
        activities: [
          "Drive to Badrinath via Joshimath",
          "Evening visit to Badrinath Temple",
        ],
      },
      {
        day: 4,
        title: "Badrinath to Joshimath",
        description:
          "Explore nearby sites around Joshimath including Mana Village, Vyas Gufa, Bhimkund, and Charanpaduka; overnight stay in Joshimath.",
        activities: [
          "Visit Mana Village",
          "Explore Vyas Gufa, Bhimkund, Charanpaduka",
          "Overnight stay in Joshimath",
        ],
      },
      {
        day: 5,
        title: "Joshimath to Rishikesh",
        description:
          "Drive to Rishikesh visiting Sriningsh Temple and Shankaracharya Math; overnight stay in Rishikesh.",
        activities: [
          "Drive to Rishikesh via Sriningsh Temple & Shankaracharya Math",
          "Overnight stay in Rishikesh",
        ],
      },
      {
        day: 6,
        title: "Rishikesh to Delhi via Haridwar",
        description:
          "Drive back to Delhi via Haridwar with drop at railway station or airport.",
        activities: [
          "Drive from Rishikesh to Delhi via Haridwar",
          "Drop at railway station / airport",
        ],
      },
    ],
    contact: {
      phone: ["+91-9412072802", "+91-9456-332153", "8630090671"],
      email: ["indiaholidayhome@gmail.com", "amitbajaj13@hotmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
    otherDetails: {
      availability: "Jan 16 – Dec 31", // tour is available during this period :contentReference[oaicite:0]{index=0}
      minAge: "9+", // required minimum age :contentReference[oaicite:1]{index=1}
      maxPeople: "34", // maximum group size :contentReference[oaicite:2]{index=2}
      region: "Uttarakhand", // the state/region where the tour takes place :contentReference[oaicite:3]{index=3}
    },
  },
  {
    id: "kedarnath-badrinath-yatra",
    title: "Kedarnath Badrinath Yatra",
    description:
      "Kedarnath Badrinath (or Do Dham / Kedar-Badri Yatra) is a beautiful introductory trip of the Himalayas. There are three categories of packages: Luxury, Deluxe, Budget. The tour includes Haridwar Ganga Aarti, Delhi arrival/departure, and spiritual / pilgrimage elements along with Himalayan scenic views.",
    duration: "8 Days / 7 Nights",
    // price: "", // the page gives sample/starting packages but doesn’t list a specific fixed price here
    image: "https://www.chardhamtour.in/blog/wp-content/uploads/2024/02/Badrinath-Kedarnath-Yatra-from-Delhi.jpg", // images are present but a single featured image URL not clearly extracted
    rating: "", // no star rating or review count shown explicitly on this page
    locations: [
      "Delhi",
      "Haridwar",
      "Guptkashi",
      "Gaurikund",
      "Kedarnath",
      "Rudraprayag",
      "Badrinath",
      "Joshimath",
      "Rishikesh",
    ],
    category: "Spiritual",
    featured: false,
    highlights: [
      "Ganga Aarti at Haridwar & Rishikesh",
      'Spiritual pilgrimage to both Kedarnath and Badrinath in one tour ("Do Dham")',
      "Three categories of accommodation: Luxury, Deluxe, Budget",
      "Scenic Himalayan views throughout",
      "Trek (or pony / doli / helicopter options) up to Kedarnath with overnight stay",
    ],
    itinerary: [
      {
        day: 1,
        title: "Delhi to Haridwar",
        description:
          "Drive from Delhi to Haridwar and attend the evening Ganga Aarti at Har-ki-Pauri.",
        activities: [
          "Drive from Delhi to Haridwar",
          "Evening Ganga Aarti at Har-ki-Pauri",
        ],
      },
      {
        day: 2,
        title: "Haridwar to Guptkashi",
        description:
          "Travel from Haridwar to Guptkashi via Devprayag and Rudraprayag, enjoying scenic views en route.",
        activities: [
          "Drive from Haridwar to Guptkashi",
          "Visit Devprayag & Rudraprayag en route",
        ],
      },
      {
        day: 3,
        title: "Guptkashi to Kedarnath",
        description:
          "Proceed to Gaurikund and trek/pony/doli/helicopter 14 km to Kedarnath; overnight stay at Kedarnath.",
        activities: [
          "Drive to Gaurikund",
          "Trek/pony/doli/helicopter to Kedarnath",
          "Overnight stay at Kedarnath",
        ],
      },
      {
        day: 4,
        title: "Kedarnath to Rudraprayag",
        description:
          "Descend from Kedarnath to Gaurikund and drive to Rudraprayag; overnight stay at Rudraprayag.",
        activities: [
          "Descend from Kedarnath to Gaurikund",
          "Drive to Rudraprayag",
          "Overnight stay at Rudraprayag",
        ],
      },
      {
        day: 5,
        title: "Rudraprayag to Badrinath",
        description:
          "Drive to Badrinath via Joshimath and attend evening Darshan at Badrinath Temple.",
        activities: [
          "Drive to Badrinath via Joshimath",
          "Evening Darshan at Badrinath Temple",
        ],
      },
      {
        day: 6,
        title: "Badrinath to Joshimath",
        description:
          "Sightseeing around Joshimath including Mana Village, Vyas Gufa, Charanpaduka, etc.",
        activities: [
          "Visit Mana Village",
          "Explore Vyas Gufa",
          "Visit Charanpaduka and nearby sites",
          "Overnight stay in Joshimath",
        ],
      },
      {
        day: 7,
        title: "Joshimath to Rishikesh",
        description:
          "Drive to Rishikesh via Shankaracharya Math and Narsingh Temple; overnight stay in Rishikesh.",
        activities: [
          "Drive to Rishikesh via Shankaracharya Math & Narsingh Temple",
          "Overnight stay in Rishikesh",
        ],
      },
      {
        day: 8,
        title: "Rishikesh to Delhi via Haridwar",
        description:
          "Return to Delhi via Haridwar with transfer to railway station or airport.",
        activities: [
          "Drive from Rishikesh to Delhi via Haridwar",
          "Transfer to railway station / airport",
        ],
      },
    ],
    otherDetails: {
      phone: ["+91-9412072802", "+91-9456332153", "8630090671"],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
  },

  {
    id: "yamunotri-gangotri-yatra",
    title: "Yamunotri – Gangotri Yatra",
    description:
      "Yamunotri Gangotri Yatra is a beautiful and introductory trip of the Himalayas. The tour offers three categories of packages: Luxury, Deluxe, and Budget, each providing a unique experience of the sacred shrines of Yamunotri and Gangotri.",
    duration: "8 Days / 7 Nights",
    // price: "", // Price details are not explicitly mentioned on the page
    image: "https://www.shikhar.com/images/gallery/tours/631/847934040.jpg", // Featured image URL is not provided on the page
    rating: "", // No star rating or review count is available
    locations: [
      "Delhi",
      "Haridwar",
      "Barkot",
      "Yamunotri",
      "Uttarkashi",
      "Gangotri",
      "Rishikesh",
    ],
    category: "Spiritual",
    featured: false,
    highlights: [
      "Ganga Aarti at Haridwar and Rishikesh",
      "Introduction to Indian spirituality",
      "Panoramic views of the Himalayas",
      "Visit to Yamunotri and Gangotri temples",
      "Three accommodation categories: Luxury, Deluxe, Budget",
    ],
    itinerary: [
      {
        day: 1,
        title: "Delhi to Haridwar",
        description: "Drive 230 km (~6 hrs) from Delhi to Haridwar.",
        activities: [
          "Drive from Delhi to Haridwar",
          "Check-in at hotel",
          "Evening at leisure",
        ],
      },
      {
        day: 2,
        title: "Haridwar to Barkot",
        description:
          "Travel from Haridwar to Barkot via Dehradun, enjoying scenic views en route.",
        activities: [
          "Drive from Haridwar to Barkot via Dehradun",
          "Check-in at hotel",
          "Leisure in Barkot",
        ],
      },
      {
        day: 3,
        title: "Barkot to Yamunotri and Back",
        description:
          "Trek 7 km each way from Barkot to Yamunotri temple and return.",
        activities: [
          "7 km trek to Yamunotri temple",
          "Temple visit and rituals",
          "Return trek to Barkot",
        ],
      },
      {
        day: 4,
        title: "Barkot to Uttarkashi",
        description: "Drive from Barkot to Uttarkashi and check-in at hotel.",
        activities: [
          "Drive from Barkot to Uttarkashi",
          "Check-in at hotel",
          "Leisure time",
        ],
      },
      {
        day: 5,
        title: "Uttarkashi to Gangotri and Back",
        description:
          "Travel 100 km (~4 hrs) to Gangotri, perform rituals and return to Uttarkashi.",
        activities: [
          "Drive to Gangotri",
          "Holy dip and pooja at Gangotri",
          "Return to Uttarkashi",
        ],
      },
      {
        day: 6,
        title: "Uttarkashi to Rishikesh",
        description:
          "Drive from Uttarkashi to Rishikesh and check-in at hotel.",
        activities: [
          "Drive to Rishikesh",
          "Check-in at hotel",
          "Evening leisure",
        ],
      },
      {
        day: 7,
        title: "Rishikesh to Haridwar",
        description:
          "Travel from Rishikesh to Haridwar, visit local sites if time permits.",
        activities: [
          "Drive from Rishikesh to Haridwar",
          "Optional sightseeing",
          "Check-in at hotel",
        ],
      },
      {
        day: 8,
        title: "Haridwar to Delhi",
        description:
          "Return to Delhi from Haridwar with drop at railway station or airport.",
        activities: [
          "Drive from Haridwar to Delhi",
          "Transfer to railway station / airport",
        ],
      },
      {
        bestTime: "April to June and September to November",
      },
    ],
    contact: {
      phone: ["+91‑9412072802", "+91‑9456332153", "8630090671"],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
  },
];
export const Honeymoon = [
  {
    id: "royal-honeymoon-india",
    title: "Royal Honeymoon in India",
    description:
      "A romantic tour for newlyweds exploring India’s royal heritage, historic cities, and scenic destinations. Experience luxury, culture, and unforgettable moments together.",
    duration: "9 Nights / 10 Days",
    // price: "On Request",
    image: "https://plus.unsplash.com/premium_photo-1661962404003-e0ca40da40ef?q=80&w=870&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    rating: "",
    locations: ["Delhi", "Agra", "Samode", "Jaipur", "Udaipur"],
    category: "Honeymoon",
    featured: false,
    highlights: [
      "Visit royal palaces and forts in Rajasthan",
      "Camel safaris and boat rides on Lake Pichola",
      "Explore historic sites like Taj Mahal, Fatehpur Sikri, Amber Fort",
      "Enjoy light and sound shows, shopping, and cultural experiences",
      "Tailored activities for a personalized honeymoon experience",
    ],
    itinerary: [
      {
        day: 1,
        title: "Arrival in Delhi",
        description: "Arrival at Delhi airport, transfer to hotel.",
        activities: [
          "Arrival at Delhi airport",
          "Transfer to hotel for night halt",
        ],
      },
      {
        day: 2,
        title: "Delhi sightseeing",
        description: "Explore Old and New Delhi.",
        activities: [
          "Visit Old Delhi (Shah Jahanabad) and Jama Masjid",
          "Rickshaw ride through Chandni Chowk",
          "Visit Raj Ghat, India Gate, Humayun’s Tomb, Qutub Minar complex",
          "Evening Sound and Light Show at Red Fort",
          "Overnight stay in city hotel",
        ],
      },
      {
        day: 3,
        title: "Agra",
        description: "Drive to Agra, visit Taj Mahal and Agra Fort.",
        activities: [
          "Drive from Delhi to Agra (203 kms)",
          "Visit Taj Mahal, Agra Fort, Akbar’s Tomb, Tomb of Itmad-Ud-Daulah",
          "Overnight stay in city hotel",
        ],
      },
      {
        day: 4,
        title: "Samode",
        description: "Drive to Samode via Fatehpur Sikri.",
        activities: [
          "Drive to Samode (270 kms)",
          "Visit Fatehpur Sikri on the way",
          "Leisure activities in Samode",
          "Overnight stay in hotel",
        ],
      },
      {
        day: 5,
        title: "Jaipur",
        description: "Sightseeing in Jaipur.",
        activities: [
          "Morning sightseeing of Samode palaces",
          "Transfer to Jaipur (42 kms)",
          "Check-in at hotel and overnight stay",
        ],
      },
      {
        day: 6,
        title: "Jaipur sightseeing",
        description:
          "Visit Amber Fort, City Palace, Hawa Mahal, Jantar Mantar.",
        activities: [
          "Visit Amber Fort, Hawa Mahal, City Palace, Jantar Mantar",
          "Stroll and shop in local bazaar",
          "Overnight stay at hotel",
        ],
      },
      {
        day: 7,
        title: "Udaipur",
        description: "Drive to Udaipur and enjoy boat ride.",
        activities: [
          "Drive to Udaipur, check-in hotel",
          "Afternoon boat ride on Lake Pichola",
          "Overnight stay at city hotel",
        ],
      },
      {
        day: 8,
        title: "Udaipur sightseeing",
        description: "Explore Udaipur’s palaces and gardens.",
        activities: [
          "Visit City Palace, Lake Palace, Saheliyon ki Baoli",
          "Evening at leisure",
          "Overnight stay at hotel",
        ],
      },
      {
        day: 9,
        title: "Return to Delhi",
        description: "Transfer back to Delhi.",
        activities: [
          "Transfer to Delhi airport",
          "Day free for leisure activities",
          "Overnight stay at hotel",
        ],
      },
      {
        day: 10,
        title: "Departure",
        description: "End of tour.",
        activities: [
          "Free day for activities of your choice",
          "Evening transfer to airport for onward journey",
        ],
      },
    ],
    contact: {
      phone: "+91-9412072802, +91-9456 332153, 8630090671",
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
    },
  },
  {
    id: "beach-honeymoon-andaman",
    title: "Beach Honeymoon in India – Andaman Package",
    description:
      "A romantic beach honeymoon for couples, exploring the pristine beaches, water sports, and scenic islands of Andaman and Nicobar.",
    duration: "5 Nights / 6 Days",
    // price: "On Request",
    image: "https://plus.unsplash.com/premium_photo-1661962958462-9e52fda9954d?q=80&w=870&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    rating: "",
    locations: ["Port Blair", "Havelock"],
    category: "Honeymoon",
    featured: false,
    highlights: [
      "Sound and Light show at Cellular Jail, Port Blair",
      "Radhanagar Beach visit – Best Beach in Asia",
      "Excursion to Elephant Beach with snorkeling and marine life exploration",
      "Kalapathar Beach visit",
      "Ferry rides across the Andaman islands and relaxed beach leisure",
    ],
    itinerary: [
      {
        day: 1,
        title: "Arrival in Port Blair",
        description: "Arrive at Port Blair and check in.",
        activities: [
          "Arrive at Veer Savarkar Airport, transfer to hotel",
          "Relax at hotel",
          "Evening Sound and Light Show at Cellular Jail",
        ],
      },
      {
        day: 2,
        title: "Transfer to Havelock",
        description: "Ferry to Havelock and visit Radhanagar Beach.",
        activities: [
          "Morning ferry from Port Blair to Havelock (approx. 2.5 hours)",
          "Check-in at hotel",
          "Visit Radhanagar Beach (Beach No 7), enjoy sunset",
          "Return to hotel and relax",
        ],
      },
      {
        day: 3,
        title: "Elephant Beach Excursion",
        description: "Snorkeling and beach fun at Elephant Beach.",
        activities: [
          "Excursion to Elephant Beach (boat ride approx. 25 mins)",
          "Snorkeling at shallow coral reefs",
          "Relax and enjoy the beach",
          "Return to hotel in Havelock",
        ],
      },
      {
        day: 4,
        title: "Leisure Day",
        description: "Free day at Havelock Island.",
        activities: [
          "Day at leisure on the beach",
          "Optional activities or self-exploration of Havelock Island",
          "Relax and enjoy scenic views",
        ],
      },
      {
        day: 5,
        title: "Return to Port Blair",
        description: "Visit Kalapathar Beach and return to Port Blair.",
        activities: [
          "Check out from hotel after breakfast",
          "Visit Kalapathar Beach",
          "Ferry back to Port Blair in the evening",
          "Check-in at hotel and relax",
        ],
      },
      {
        day: 6,
        title: "Departure",
        description: "End of tour.",
        activities: [
          "Transfer to airport for departure",
          "End of tour with affectionate memories",
        ],
      },
    ],
    contact: {
      phone: "+91-9412072802, +91-9456 332153, 8630090671",
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
    },
  },
  {
    id: "kashmir-honeymoon-tour",
    title: "Kashmir Honeymoon Tour",
    description:
      "A serene honeymoon tour in Kashmir where couples can enjoy the natural beauty of Srinagar, Gulmarg, Sonmarg, and Pahalgam, creating unforgettable memories amidst picturesque landscapes.",
    duration: "5 Nights / 6 Days",
    // price: "On Request",
    image: "https://images.unsplash.com/photo-1595815771614-ade9d652a65d?q=80&w=870&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    rating: "",
    locations: ["Srinagar", "Gulmarg", "Sonmarg", "Pahalgam"],
    category: "Honeymoon",
    featured: false,
    highlights: [
      "Shikara ride on Dal Lake",
      "Mughal Gardens in Srinagar",
      "Gondola Ride in Gulmarg",
      "Thajiwas Glacier in Sonmarg",
      "Saffron Fields in Pahalgam",
    ],
    itinerary: [
      {
        day: 1,
        title: "Arrival in Srinagar",
        description:
          "Arrival at Srinagar Airport and transfer to resort or houseboat.",
        activities: [
          "Arrival at Srinagar Airport",
          "Transfer to resort or houseboat",
          "Shikara ride on Dal Lake",
          "Sightseeing: Kabutarkhana, Bathing Boats, Char Chinar Island, Floating Gardens, Floating Shops",
        ],
      },
      {
        day: 2,
        title: "Srinagar Sightseeing",
        description: "Half-day city tour of Srinagar.",
        activities: [
          "Half-day city tour",
          "Visit Nishat Bagh and Shalimar Bagh (Mughal Gardens)",
          "Visit Shankaracharya Temple",
          "Return to hotel/houseboat in evening",
        ],
      },
      {
        day: 3,
        title: "Gulmarg Excursion",
        description: "Day trip to Gulmarg.",
        activities: [
          "Drive to Gulmarg",
          "Explore the place on your own",
          "Optional Gondola Ride / Horse Riding",
          "Return to Srinagar in evening",
        ],
      },
      {
        day: 4,
        title: "Sonmarg Excursion",
        description: "Day trip to Sonmarg.",
        activities: [
          "Drive to Sonmarg, the Meadow of Gold",
          "Stop at scenic points along the way",
          "Visit Thajiwas Glacier",
          "Return to Srinagar in evening",
        ],
      },
      {
        day: 5,
        title: "Pahalgam Excursion",
        description: "Day trip to Pahalgam.",
        activities: [
          "Drive to Pahalgam, the Vale of Kashmir",
          "Visit Avantipur ruins and Saffron Fields",
          "Return to Srinagar in evening",
        ],
      },
      {
        day: 6,
        title: "Departure",
        description: "Check out and transfer to Srinagar Airport.",
        activities: [
          "Check out from hotel/houseboat after breakfast",
          "Transfer to Srinagar Airport for onward journey",
        ],
      },
    ],
    contact: {
      phone: "+91-9412072802, +91-9456 332153, 8630090671",
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
    },
  },
  {
    id: "himachal-pradesh-honeymoon",
    title: "Honeymoon in Himachal Pradesh",
    description:
      "A serene and romantic honeymoon experience in Himachal Pradesh for newly married couples, surrounded by snow-capped mountains, lush forests, and tranquil hill stations.",
    duration: "On Request",
    // price: "On Request",
    image: "https://images.unsplash.com/photo-1597074866923-dc0589150358?q=80&w=870&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    rating: "",
    locations: [
      "Shimla",
      "Kullu",
      "Manali",
      "Kasauli",
      "Solan",
      "Dharamshala",
      "Chamba",
      "Chail",
    ],
    category: "Honeymoon",
    featured: false,
    highlights: [
      "Pick and drop services",
      "Luxury accommodation in special honeymoon suites",
      "Sightseeing at prime destinations and hill stations",
      "Multi-cuisine meals",
      "Tour guide on demand",
      "Adventure and sports activities",
      "Exploration of deep valleys, glaciers, and exotic wildlife",
    ],
    itinerary: [
      {
        day: 1,
        title: "Arrival",
        description: "Reach Shimla / Manali and check-in at hotel.",
        activities: [
          "Pick up from airport/railway station",
          "Transfer to hotel",
          "Relax and enjoy the surroundings",
        ],
      },
      {
        day: 2,
        title: "Shimla Sightseeing",
        description: "Explore Shimla and nearby attractions.",
        activities: [
          "Local sightseeing in Shimla",
          "Visit Mall Road, Ridge, Jakhoo Temple",
          "Overnight stay in hotel",
        ],
      },
      {
        day: 3,
        title: "Kullu & Manali",
        description: "Drive to Kullu and Manali for sightseeing.",
        activities: [
          "Visit temples and scenic spots in Kullu",
          "Continue drive to Manali",
          "Check-in and overnight stay",
        ],
      },
      {
        day: 4,
        title: "Manali Sightseeing",
        description: "Explore Manali and adventure activities.",
        activities: [
          "Visit Hadimba Temple, Solang Valley",
          "Optional adventure sports (paragliding, skiing)",
          "Overnight stay in hotel",
        ],
      },
      {
        day: 5,
        title: "Dharamshala / McLeodganj",
        description: "Excursion to Dharamshala.",
        activities: [
          "Drive to Dharamshala / McLeodganj",
          "Visit Dalai Lama Temple and Bhagsunag Waterfall",
          "Overnight stay in hotel",
        ],
      },
      {
        day: 6,
        title: "Chamba & Chail",
        description: "Explore Chamba and Chail.",
        activities: [
          "Drive to Chamba",
          "Visit Chamba Palace, temples",
          "Continue to Chail and visit Chail Palace",
          "Overnight stay in hotel",
        ],
      },
      {
        day: 7,
        title: "Departure",
        description: "End of tour and transfer to airport/railway station.",
        activities: [
          "Check out from hotel",
          "Transfer to airport/railway station for onward journey",
        ],
      },
    ],
    contact: {
      phone: "+91-9412072802, +91-9456 332153, 8630090671",
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
    },
  },

  {
    id: "rajasthan-honeymoon-tours",
    title: "Rajasthan Honeymoon Tours",
    description:
      "Rajasthan offers newlyweds a mix of royal heritage, romantic lakes, deserts, forts, palaces, and cultural experiences. Couples can enjoy boat rides, shopping, camel safaris, sound & light shows, and more.",
    duration: "On Request",
    // price: "On Request",
    image: "https://plus.unsplash.com/premium_photo-1661904165347-369200d4bf72?q=80&w=870&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    rating: "",
    locations: [
      "Delhi",
      "Agra",
      "Jaipur",
      "Udaipur",
      "Samode",
      "Jodhpur",
      "Jaisalmer",
      "Ranthambore",
    ],
    category: "Honeymoon",
    featured: false,
    highlights: [
      "Visit Humayun’s Tomb and Jama Masjid, Delhi",
      "Explore Akbar’s Tomb and Taj Mahal, Agra",
      "Sound & light show at Red Fort, Delhi",
      "Shop in Jaipur’s local bazaars",
      "Day trip to Amer Fort and Jantar Mantar, Jaipur",
      "Guided tour of Samode’s Palaces",
      "Excursion to Fatehpur Sikri",
      "Boat ride on Lake Pichola, Udaipur",
    ],
    itinerary: [
      {
        day: 1,
        title: "Arrival in Delhi",
        description: "Reach Delhi and check-in at hotel.",
        activities: [
          "Arrival at airport",
          "Transfer to hotel",
          "Relax and evening at leisure",
        ],
      },
      {
        day: 2,
        title: "Delhi Sightseeing",
        description: "Explore Delhi’s heritage sites.",
        activities: [
          "Visit Humayun’s Tomb, Jama Masjid",
          "Rickshaw ride in Chandni Chowk",
          "Evening Sound & Light Show at Red Fort",
        ],
      },
      {
        day: 3,
        title: "Delhi to Agra",
        description: "Drive to Agra and visit Taj Mahal and Agra Fort.",
        activities: [
          "Drive to Agra",
          "Visit Taj Mahal and Agra Fort",
          "Overnight stay in Agra",
        ],
      },
      {
        day: 4,
        title: "Agra to Jaipur via Fatehpur Sikri",
        description: "Visit Fatehpur Sikri on the way to Jaipur.",
        activities: [
          "Visit Fatehpur Sikri",
          "Drive to Jaipur",
          "Check-in and overnight stay",
        ],
      },
      {
        day: 5,
        title: "Jaipur Sightseeing",
        description: "Explore Amber Fort, City Palace, Hawa Mahal.",
        activities: [
          "Visit Amber Fort, City Palace, Jantar Mantar, Hawa Mahal",
          "Shop in local bazaars",
          "Overnight stay",
        ],
      },
      {
        day: 6,
        title: "Jaipur to Udaipur",
        description: "Drive to Udaipur and enjoy boat ride.",
        activities: [
          "Drive to Udaipur",
          "Check-in at hotel",
          "Boat ride on Lake Pichola",
          "Overnight stay",
        ],
      },
      {
        day: 7,
        title: "Udaipur Sightseeing",
        description: "Visit City Palace and other attractions.",
        activities: [
          "Visit City Palace, Saheliyon ki Baoli, Lake Palace",
          "Evening at leisure",
          "Overnight stay",
        ],
      },
      {
        day: 8,
        title: "Departure",
        description: "End of tour and transfer to airport/railway station.",
        activities: [
          "Check out from hotel",
          "Transfer to airport/railway station",
        ],
      },
    ],
    contact: {
      phone: "+91-9412072802, +91-9456 332153, 8630090671",
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
    },
  },

  {
    id: "south-india-honeymoon",
    title: "Honeymoon in South India",
    description:
      "A romantic getaway for newlyweds exploring the serene beaches, lush hills, tranquil forests, and backwaters of South India. Perfect for couples seeking peace, privacy, and memorable experiences.",
    duration: "On Request",
    // price: "On Request",
    image: "https://plus.unsplash.com/premium_photo-1697729603226-8c8627dfc12b?q=80&w=870&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    rating: "",
    locations: [
      "Pondicherry",
      "Kanyakumari",
      "Coonoor",
      "Kodaikanal",
      "Ooty",
      "Coorg",
      "Thekkady",
      "Kabini",
      "Rishikonda Beach",
      "Bekal",
      "Kumarakom",
      "Kovalam",
      "Wayanad",
      "Alleppey",
      "Munnar",
    ],
    category: "Honeymoon",
    featured: false,
    highlights: [
      "Tranquil and romantic destinations for couples",
      "Beaches, hills, forests, and backwaters",
      "Opportunities for adventure, nature walks, and cultural experiences",
      "Privacy and peaceful surroundings",
      "Ideal for post-wedding bonding and honeymoon memories",
    ],
    itinerary: [
      {
        day: 1,
        title: "Arrival in South India",
        description: "Reach first destination and check-in.",
        activities: [
          "Pick up from airport/railway station",
          "Check-in at hotel",
          "Relax and evening at leisure",
        ],
      },
      {
        day: 2,
        title: "Pondicherry & Kanyakumari",
        description: "Explore Pondicherry and Kanyakumari.",
        activities: [
          "Visit Paradise Beach, Auroville Ashram",
          "Vivekananda Rock Memorial, Thiruvalluvar Statue",
        ],
      },
      {
        day: 3,
        title: "Coonoor & Kodaikanal",
        description: "Explore hill stations.",
        activities: [
          "Visit Sim’s Park, Lamb’s Rock, Dolphin’s Nose",
          "Visit Kodai Lake, Green Valley View, Pillar Rocks",
        ],
      },
      {
        day: 4,
        title: "Ooty & Coorg",
        description: "Explore Ooty and Coorg.",
        activities: [
          "Visit Ooty Lake, Botanical Gardens, Nilgiri Mountain Railway",
          "Explore Abbey Falls, Talacauvery, Nagarhole National Park",
        ],
      },
      {
        day: 5,
        title: "Thekkady & Kabini",
        description: "Wildlife and backwater experiences.",
        activities: [
          "Periyar National Park, Bamboo Rafting",
          "Elephant Safari, Jungle Safari, Boating",
        ],
      },
      {
        day: 6,
        title: "Kerala Backwaters & Beaches",
        description: "Explore Kerala backwaters and beaches.",
        activities: [
          "Houseboat stay in Alleppey/Kumarakom",
          "Kovalam Lighthouse Beach, Hawah Beach",
          "Bekal Fort and Beach",
        ],
      },
      {
        day: 7,
        title: "Munnar & Wayanad",
        description: "Tea gardens and scenic hills.",
        activities: [
          "Visit Rose Garden, Echo Point, Elephant Arrival Spot in Munnar",
          "Banasura Dam, Pookot Lake, Chembra Peak in Wayanad",
        ],
      },
      {
        day: 8,
        title: "Departure",
        description: "End of tour and transfer to airport/railway station.",
        activities: [
          "Check out from hotel",
          "Transfer to airport/railway station",
        ],
      },
    ],
    contact: {
      phone: "+91-9412072802, +91-9456 332153, 8630090671",
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
    },
  },

  {
    id: "exotic-honeymoon-india",
    title: "Exotic Honeymoon in India",
    description:
      "An exhilarating and romantic honeymoon experience for couples, exploring exotic and breathtaking destinations across India, combining culture, nature, and memorable experiences.",
    duration: "Customizable",
    // price: "On Request",
    image: "https://images.unsplash.com/photo-1674922049190-f631ba5bc44f?q=80&w=869&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    rating: "",
    locations: ["Kashmir", "Kerala", "Rajasthan", "Sikkim"],
    category: "Honeymoon",
    featured: false,
    highlights: [
      "Unforgettable romantic experiences for newlyweds",
      "Domestic honeymoon packages across India",
      "Explore snow-capped mountains, pristine beaches, and cultural heritage",
      "Personalized itineraries based on destination and budget",
      "Opportunities to experience diverse Indian cultures and lifestyles",
    ],
    itinerary: [
      {
        day: 1,
        title: "Custom Itinerary",
        description: "Choose your preferred destinations and plan.",
        activities: [
          "Consult with travel planner",
          "Customize itinerary based on interests and budget",
        ],
      },
    ],
    contact: {
      phone: "+91-9412072802, +91-9456 332153, 8630090671",
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
    },
  },
  {
    id: "romantic-pachmarhi",
    title: "Romantic Pachmarhi",
    description:
      "A short and sweet honeymoon trip to Pachmarhi, the only hill station in Madhya Pradesh, known for its waterfalls, caves, and lush greenery.",
    duration: "4 Days / 3 Nights",
    // price: "On Request",
    image:
      "https://images.unsplash.com/photo-1690441850698-b9aae3786d35?q=80&w=774&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    rating: 4.6,
    locations: ["Pipariya", "Pachmarhi", "Pipariya"],
    category: "Honeymoon",
    featured: false,
    highlights: [
      "Visit famous waterfalls like Bee Falls and Apsara Vihar",
      "Explore ancient caves and rock shelters",
      "Romantic walks amidst lush green forests",
      "Beautiful sunset views from Dhupgarh – the highest point in MP",
      "Comfortable honeymoon-friendly accommodations",
    ],
    itinerary: [
      {
        day: 1,
        title: "Arrival at Pipariya – Transfer to Pachmarhi",
        description:
          "Arrive at Pipariya railway station. Meet our representative and transfer to Pachmarhi. Check-in at the hotel and spend the evening at leisure.",
        activities: [
          "Arrival at Pipariya",
          "Drive to Pachmarhi",
          "Hotel check-in",
          "Evening leisure walk",
        ],
      },
      {
        day: 2,
        title: "Pachmarhi Sightseeing – Waterfalls and Caves",
        description:
          "Explore popular waterfalls and natural sites of Pachmarhi, perfect for couples.",
        activities: [
          "Bee Falls",
          "Apsara Vihar",
          "Pandava Caves",
          "Handi Khoh",
        ],
      },
      {
        day: 3,
        title: "Dhupgarh Sunset & Nature Trails",
        description:
          "Enjoy romantic nature trails and visit Dhupgarh, the highest point of Madhya Pradesh, for breathtaking sunset views.",
        activities: [
          "Gupt Mahadev",
          "Jata Shankar Cave",
          "Romantic walk in Satpura forest",
          "Dhupgarh sunset view",
        ],
      },
      {
        day: 4,
        title: "Departure from Pachmarhi",
        description:
          "After breakfast, check out from hotel and drive back to Pipariya railway station for onward journey.",
        activities: [
          "Breakfast at hotel",
          "Check-out",
          "Drive to Pipariya",
          "Trip ends",
        ],
      },
    ],
  },
  {
    id: "kerala-backwater-honeymoon",
    title: "Kerala Backwater Honeymoon in India",
    description:
      "Romantic 7-day honeymoon tour through Kerala featuring treehouse stays, a houseboat cruise, and serene beaches.",
    duration: "7 Days / 6 Nights",
    // price: "On Request",
    image:
      "https://plus.unsplash.com/premium_photo-1697729438401-fcb4ff66d9a8?q=80&w=1470&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    rating: 4.9,
    locations: ["Cochin", "Munnar", "Alleppey", "Kovalam", "Trivandrum"],
    category: "Honeymoon",
    featured: true,
    highlights: [
      "Sightseeing in Fort Kochi including St. Francis Church",
      "Special Kathakali performance in Cochin",
      "Stay in a treehouse in Munnar",
      "Visit Eravikulam National Park",
      "Overnight houseboat experience in Alleppey",
      "Relax at Kovalam’s crescent-shaped beaches",
      "Visit Padmanabhaswamy Temple in Trivandrum",
    ],
    itinerary: [
      {
        day: 1,
        title: "Arrival in Cochin",
        description:
          "Arrive at Cochin Airport/Railway Station. Transfer to hotel, check-in, and relax. Later, explore Fort Cochin and visit St. Francis Church, Dutch Palace, and Jewish Synagogue. In the evening, enjoy a Kathakali dance performance.",
        activities: [
          "Arrival at Cochin",
          "Hotel check-in",
          "Fort Cochin sightseeing",
          "Kathakali dance show",
        ],
      },
      {
        day: 2,
        title: "Cochin to Munnar",
        description:
          "Drive to Munnar and check-in at a luxurious treehouse resort. Spend the rest of the day at leisure or explore the surroundings.",
        activities: [
          "Breakfast at hotel",
          "Drive to Munnar",
          "Check-in at treehouse resort",
          "Leisure or local exploration",
        ],
      },
      {
        day: 3,
        title: "Munnar Sightseeing",
        description:
          "Full day sightseeing of Munnar covering Mattupetty Dam, Old Munnar Town, Town Market, and Eravikulam National Park. Return to treehouse stay.",
        activities: [
          "Mattupetty Dam",
          "Old Munnar Town",
          "Town Market",
          "Eravikulam National Park",
        ],
      },
      {
        day: 4,
        title: "Munnar to Alleppey (Houseboat Stay)",
        description:
          "Drive to Alleppey and board a traditional houseboat (Kettuvallam) for an overnight backwater cruise with sightseeing.",
        activities: [
          "Breakfast at resort",
          "Drive to Alleppey",
          "Check-in at houseboat",
          "Backwater cruise",
        ],
      },
      {
        day: 5,
        title: "Alleppey to Kovalam",
        description:
          "Disembark from the houseboat after breakfast and drive to Kovalam. Check-in at a beach resort and relax by the crescent-shaped beaches.",
        activities: [
          "Houseboat breakfast",
          "Drive to Kovalam",
          "Check-in at beach resort",
          "Leisure at Kovalam beach",
        ],
      },
      {
        day: 6,
        title: "Kovalam Sightseeing",
        description:
          "Explore local attractions including Padmanabhaswamy Temple, Veli Tourist Village, Museum Complex, Science & Technology Museum, and Shankumugham Beach. Return to Kovalam.",
        activities: [
          "Padmanabhaswamy Temple",
          "Veli Tourist Village",
          "Museum Complex",
          "Science & Technology Museum",
          "Shankumugham Beach",
        ],
      },
      {
        day: 7,
        title: "Departure from Trivandrum",
        description:
          "After breakfast, check-out and transfer to Trivandrum Airport/Railway Station for onward journey.",
        activities: [
          "Breakfast at resort",
          "Check-out",
          "Drive to Trivandrum",
          "Trip ends",
        ],
      },
    ],
  },
  {
    id: "honeymoon-adventure-tours",
    title: "Honeymoon With Adventures Tours",
    description:
      "Top 15 adventure honeymoon destinations in India where thrill meets romance. From the azure beaches of Lakshadweep to the snowy slopes of Auli, these destinations blend adrenaline and intimacy for newlyweds seeking an unforgettable honeymoon.",
    duration: "Customizable",
    // price: "On Request",
    image:
      "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?q=80&w=1200",
    rating: 4.9,
    locations: [
      "Lakshadweep",
      "Leh & Ladakh",
      "Manali",
      "Jaisalmer",
      "Gulmarg",
      "Shillong",
      "Goa",
      "Jim Corbett National Park",
      "Darjeeling",
      "Lonavala",
      "Andaman & Nicobar Islands",
      "Matheran",
      "Rishikesh",
      "Rann of Kutch",
      "Auli",
    ],
    category: "Honeymoon",
    featured: true,
    highlights: [
      "Scuba diving in Lakshadweep’s azure waters",
      "Trekking and camping in Leh & Ladakh",
      "Skiing and paragliding in Manali",
      "Camel safari across Jaisalmer’s golden dunes",
      "Heli-skiing in Gulmarg",
      "Water sports in Goa",
      "Wildlife safari in Jim Corbett",
      "Toy train and tea gardens in Darjeeling",
      "Hot-air ballooning in Lonavala",
      "Snorkeling and parasailing in Andaman",
      "River rafting in Rishikesh",
      "Romantic camel rides in Rann of Kutch",
      "Asia’s longest cable car ride in Auli",
    ],
    itinerary: [
      {
        day: 1,
        title: "Lakshadweep",
        description:
          "Relax on tranquil beaches and dive into thrilling water sports in Lakshadweep.",
        activities: [
          "Scuba diving",
          "Snorkeling",
          "Kayaking",
          "Speed boating",
          "Canoeing",
        ],
        best_time_to_visit: "October to May",
      },
      {
        day: 2,
        title: "Leh & Ladakh",
        description:
          "Adventure meets serenity in Ladakh with high mountain passes and pristine lakes.",
        activities: [
          "Trekking",
          "Jeep safari",
          "Mountain biking",
          "Camping",
          "River rafting",
        ],
        best_time_to_visit: "June to September",
      },
      {
        day: 3,
        title: "Manali",
        description:
          "Snow-clad Himalayas, adventure sports, and cozy resorts make Manali a honeymoon hotspot.",
        activities: [
          "Skiing",
          "Paragliding",
          "Trekking",
          "Camping",
          "Mountain biking",
        ],
        best_time_to_visit: "September to June",
      },
      {
        day: 4,
        title: "Jaisalmer",
        description:
          "The golden city offers desert adventures, sand dunes, and romantic camel rides.",
        activities: ["Camel safari", "Jeep safari", "Desert camping"],
        best_time_to_visit: "October to February",
      },
      {
        day: 5,
        title: "Gulmarg",
        description:
          "A winter paradise famous for skiing, gondola rides, and Asia’s only heli-skiing.",
        activities: ["Skiing", "Heli-skiing", "Trekking", "Golf", "Fishing"],
        best_time_to_visit: "November to February",
      },
      {
        day: 6,
        title: "Shillong",
        description:
          "The Scotland of the East blends natural beauty with soft adventure activities.",
        activities: [
          "Kayaking",
          "Water skiing",
          "Nature walks",
          "Trekking",
          "Angling",
        ],
        best_time_to_visit: "October–November, March–April",
      },
      {
        day: 7,
        title: "Goa",
        description:
          "Golden beaches, vibrant nightlife, and exciting water adventures make Goa unforgettable.",
        activities: [
          "Scuba diving",
          "Snorkeling",
          "Kayaking",
          "Camping",
          "Fishing",
        ],
        best_time_to_visit: "October to March",
      },
      {
        day: 8,
        title: "Jim Corbett National Park",
        description:
          "India’s first national park offers wildlife adventures amidst scenic landscapes.",
        activities: [
          "Wildlife safari",
          "River rafting",
          "Rock climbing",
          "Angling",
        ],
        best_time_to_visit: "Mid November to Mid June",
      },
      {
        day: 9,
        title: "Darjeeling",
        description:
          "Tea gardens, Himalayan views, and soft adventures make Darjeeling perfect for couples.",
        activities: [
          "Trekking",
          "Whitewater rafting",
          "Paragliding",
          "Mountaineering",
        ],
        best_time_to_visit: "April to June, September to December",
      },
      {
        day: 10,
        title: "Lonavala",
        description:
          "Quick getaway from Mumbai with thrilling adventure activities and natural beauty.",
        activities: [
          "Hot-air ballooning",
          "Bungee jumping",
          "Rock climbing",
          "Flying fox",
        ],
        best_time_to_visit: "October to April",
      },
      {
        day: 11,
        title: "Andaman & Nicobar Islands",
        description:
          "Romantic island paradise offering marine adventures and secluded beaches.",
        activities: [
          "Scuba diving",
          "Snorkeling",
          "Parasailing",
          "Wind surfing",
          "Water skiing",
        ],
        best_time_to_visit: "December to March",
      },
      {
        day: 12,
        title: "Matheran",
        description:
          "A vehicle-free hill station where couples can enjoy serene nature trails.",
        activities: ["Horse riding", "Hiking", "Nature walks"],
        best_time_to_visit: "September to June",
      },
      {
        day: 13,
        title: "Rishikesh",
        description:
          "The yoga capital of the world with endless adventure sports by the Ganges.",
        activities: [
          "River rafting",
          "Bungee jumping",
          "Kayaking",
          "Mountain biking",
          "Yoga",
        ],
        best_time_to_visit: "September to June",
      },
      {
        day: 14,
        title: "Rann of Kutch",
        description:
          "Romantic white desert offering camel safaris and starlit nights.",
        activities: ["Camel safari", "Cultural experiences"],
        best_time_to_visit: "October to March",
      },
      {
        day: 15,
        title: "Auli",
        description:
          "Snowy slopes, ski resorts, and Asia’s longest cable car ride make Auli magical.",
        activities: ["Skiing", "Cable car ride", "Trekking"],
        best_time_to_visit: "November to March",
      },
    ],
    contact_info: {
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
      phone: ["+91 – 9412072802", "+91 – 9456 332153", "+91 – 8630090671"],
      emails: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
    },
  },
];
export const IndiaTour = [
  {
    id: "best-of-west-india",
    title: "Best Of West India Tours",
    description:
      "Explore the vibrant cities, beaches, temples, caves, and wildlife sanctuaries of Western India including Mumbai, Goa, Gujarat, and Maharashtra.",
    duration: "",
    // price: "",
    image: "https://images.unsplash.com/photo-1529253355930-ddbe423a2ac7?q=80&w=465&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    rating: "",
    locations: ["Mumbai", "Goa", "Gujarat", "Maharashtra"],
    category: "indiatour",
    featured: false,
    highlights: [
      "Visit the commercial capital Mumbai (Bombay) and explore its vibrant street life",
      "Relax on the palm-fringed beaches of Goa",
      "Explore the Ajanta & Ellora caves in Maharashtra",
      "Experience the fairs, festivals, and wildlife sanctuaries in Gujarat",
      "Enjoy iconic spots like Chowpatty Beach and the Taj Mahal Hotel in Mumbai",
    ],
    itinerary: [],
    contact: {
      phone: ["+91-9412072802", "+91-9456332153", "01334-246280"],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
    },
  },
  {
    id: "best-of-north-india",
    title: "Best Of North India Tours",
    description:
      "North India, which extends from Rajasthan in the west to Bihar in the East, is a region of the Indian subcontinent over which kings and empires have fought for centuries. The Northern Zone of India with Uttar Pradesh, Himachal Pradesh, Jammu & Kashmir, Uttarakhand, Punjab, Haryana, Rajasthan, Chandigarh having very much popular tourist destinations, we are suggesting few tour options below…… …",
    duration: "", // no single overall duration given
    // price: "", // no pricing given
    image: "https://images.unsplash.com/photo-1712510817140-917938f92e5b?q=80&w=774&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D", // individual images shown but no single featured image URL found
    rating: "", // no rating provided
    locations: [
      "Uttar Pradesh",
      "Himachal Pradesh",
      "Jammu & Kashmir",
      "Uttarakhand",
      "Punjab",
      "Haryana",
      "Rajasthan",
      "Chandigarh",
    ],
    category: "indiatour", // not explicitly categorized (e.g. “Cultural”, “Adventure”), so left blank
    featured: false, // not labelled “featured” on page
    highlights: [
      "Visit Agra, Mathura, Fatehpur Sikri, Lucknow, Allahabad, Varanasi, Dudhwa National Park, Jhansi, Ayodhya, Sarnath in Uttar Pradesh",
      "Explore the valleys, hill stations and passes of Himachal Pradesh including Shimla, Manali, Rohtang Pass, Kullu, Dalhousie, Dharamshala etc.",
      "Enjoy the beauty of Jammu & Kashmir: Srinagar, Gulmarg, Sonamarg, Pahalgam, Dal Lake, Mughal Gardens etc.",
      "Tour of Uttarakhand covering Haridwar, Rishikesh, Yamunotri, Gangotri, Kedarnath, Valley of Flowers & Hemkund etc.",
      "Golden Temple and religious sites of Punjab; cultural, historical & natural sites in Haryana, Delhi, Rajasthan including Jaipur, Jodhpur, Jaisalmer, Ranthambhore etc.",
    ],
    itinerary: [
      {
        name: "Ganges View Tour",
        duration: "07 Nights / 08 Days",
        destinations: [
          "Delhi",
          "Varanasi",
          "Haridwar",
          "Rishikesh",
          "Dev-Prayag",
          "Rishikesh",
          "Delhi",
        ],
        bestTime: "Around The Year",
      },
      {
        name: "Taj & Tiger Tours",
        duration: "10 Nights / 11 Days",
        destinations: [
          "Delhi",
          "Agra",
          "Fatehpur Sikri",
          "Bharatpur",
          "Ranthambhore",
          "Jaipur",
          "Delhi",
          "Corbett National Park",
          "Ramnagar",
          "Dudhwa National Park",
          "Delhi",
        ],
        bestTime: "1 October to end June",
      },
      {
        name: "Golden Triangle Tour",
        duration: "6 Nights / 7 Days",
        destinations: ["Delhi", "Agra", "Fatehpur Sikri", "Jaipur", "Delhi"],
        bestTime: "Around The Year",
      },
      {
        name: "Krishna Birth + Ganges View Tour",
        duration: "6 Nights / 7 Days",
        destinations: [
          "Delhi",
          "Haridwar",
          "Rishikesh",
          "Delhi",
          "Vrindavan",
          "Mathura",
          "Agra",
          "Varanasi",
          "Delhi",
        ],
        bestTime: "Around The Year",
      },
      {
        name: "Buddha Tours",
        duration: "11 Nights / 10 Days",
        destinations: [
          "Delhi",
          "Gonda",
          "Saraswati",
          "Lumbini",
          "Kushinagar",
          "Patna",
          "Bodh Gaya",
          "Varanasi",
          "Kanpur",
          "Agra",
        ],
        bestTime: "Around The Year",
      },
      {
        name: "Buddha Tour With Golden Triangle",
        duration: "15 Nights / 16 Days",
        destinations: [
          "Delhi",
          "Lucknow",
          "Sravasti",
          "Kapilvastu",
          "Lumbini",
          "Kushinagar",
          "Vaishali",
          "Patna",
          "Rajgir",
          "Nalanda",
          "Bodh Gaya",
          "Varanasi",
          "Sarnath",
          "Agra",
          "Jaipur",
        ],
        bestTime: "Around The Year",
      },
      {
        name: "Himalayan Panorama",
        duration: "12 Nights / 13 Days",
        destinations: [
          "Delhi",
          "Shimla",
          "Manali",
          "Rohtang Pass",
          "Kullu",
          "Manikaran",
          "Dharamshala",
          "Mc-Leodganj",
          "Dalhousie",
          "Pathankot",
          "Delhi",
        ],
        bestTime: "Around The Year",
      },
      {
        name: "Magic of Himachal",
        duration: "6 Nights / 7 Days",
        destinations: [
          "Delhi",
          "Shimla",
          "Dharamshala",
          "Dalhousie",
          "Khajjiar",
          "Chamba",
          "Dalhousie",
          "Pathankot",
          "Delhi",
        ],
        bestTime: "Around The Year",
      },
      {
        name: "Himachal & Punjab Special",
        duration: "11 Nights / 12 Days",
        destinations: [
          "Delhi",
          "Amritsar",
          "Dharmshala",
          "Manali",
          "Rohtang Pass",
          "Kullu",
          "Shimla",
          "Chail",
          "Kufri",
          "Nalderha",
          "Shimla",
          "Chandigarh",
          "Delhi",
        ],
        bestTime: "Around The Year",
      },
      {
        name: "Himachal & Uttarakhand Special",
        duration: "11 Nights / 12 Days",
        destinations: [
          "Delhi",
          "Haridwar",
          "Rishikesh",
          "Chamba",
          "Dhaulatty (or Dhaunalti?)",
          "Mussoorie",
          "Paonta Sahib",
          "Shimla",
          "Manali",
          "Rohtang Pass",
          "Kullu",
          "Chandigarh",
          "Delhi",
        ],
        bestTime: "Around The Year",
      },
      {
        name: "Honeymoon In Himachal",
        duration: "9 Nights / 10 Days",
        destinations: [
          "Delhi",
          "Shimla",
          "Dharamshala",
          "Manali",
          "Chandigarh",
          "Delhi",
        ],
        bestTime: "Around The Year",
      },
      {
        name: "Amarnath Yatra",
        duration: "12 Nights & 13 Days",
        destinations: [
          "New Delhi",
          "Jammu",
          "Srinagar",
          "Baltal",
          "Sonamarg",
          "Srinagar",
          "Dal Lake",
          "Mughal Gardens",
          "Shankaracharya Temple",
          "Katra",
          "Vaishno Devi",
          "New Delhi",
        ],
        bestTime: "July – August",
      },
      {
        name: "Vaishno Devi Yatra",
        duration: "10 Nights & 11 Days",
        destinations: [
          "New Delhi",
          "Jammu",
          "Srinagar",
          "Sonamarg",
          "Srinagar",
          "Dal Lake",
          "Mughal Gardens",
          "Shankaracharya Temple",
          "Katra",
          "Vaishno Devi",
          "New Delhi",
        ],
        bestTime: "Around The Year",
      },
      {
        name: "Best of Jammu & Kashmir",
        duration: "10 Nights / 09 Days",
        destinations: [
          "Delhi",
          "Jammu",
          "Srinagar",
          "Gulmarg",
          "Sonamarg",
          "Srinagar",
          "Pahalgam",
          "Patni Top",
          "Katra",
          "Jammu",
          "Delhi",
        ],
        bestTime: "Around The Year",
      },
      {
        name: "Kashmir + Vaishno Devi Darshan",
        duration: "8 Nights / 7 Days",
        destinations: [
          "Delhi",
          "Katra",
          "Pahalgam",
          "Srinagar",
          "Gulmarg",
          "Sonamarg",
          "Srinagar",
          "Delhi",
        ],
        bestTime: "Around The Year",
      },
      {
        name: "Kashmir + Himachal & Punjab Special",
        duration: "11 Nights / 10 Days",
        destinations: [
          "Delhi",
          "Shimla",
          "Kufri",
          "Kullu",
          "Manali",
          "Rohtang Pass",
          "Mandi",
          "Dharamshala",
          "Jammu",
          "Srinagar",
          "Sonamarg",
          "Gulmarg",
          "Srinagar",
          "Jammu",
          "Amritsar",
          "Delhi",
        ],
        bestTime: "Around The Year",
      },
      {
        name: "Kashmir + Uttarakhand Special",
        duration: "15 Nights / 16 Days",
        destinations: [
          "Delhi",
          "Haridwar",
          "Rishikesh",
          "Dehradun",
          "Mussoorie",
          "Chamba",
          "Haridwar",
          "Jammu",
          "Katra",
          "Pahalgam",
          "Srinagar",
          "Sonamarg",
          "Gulmarg",
          "Srinagar",
          "Jammu",
          "Delhi",
        ],
        bestTime: "Around The Year",
      },
      {
        name: "Char Dham Yatra",
        duration: "12 Nights / 11 Days",
        destinations: [
          "Delhi",
          "Haridwar",
          "Barkot",
          "Yamunotri",
          "Uttarkashi",
          "Gangotri",
          "Rudraprayag",
          "Kedarnath",
          "Joshimath",
          "Badrinath",
          "Rishikesh",
          "Delhi",
        ],
        bestTime: "May – October",
      },
      {
        name: "Kedarnath – Badrinath Yatra",
        duration: "6 Nights / 7 Days",
        destinations: [
          "Delhi",
          "Haridwar",
          "Rudraprayag",
          "Kedarnath",
          "Joshimath",
          "Badrinath",
          "Rishikesh",
          "Delhi",
        ],
        bestTime: "May – October",
      },
      {
        name: "Valley of Flowers & Hemkund Trek",
        duration: "6 Nights / 7 Days",
        destinations: [
          "Delhi",
          "Haridwar",
          "Joshimath",
          "Badrinath",
          "Govindghat",
          "Ghangaria",
          "Valley of Flowers",
          "Hemkund",
          "Joshimath",
          "Rishikesh",
          "Delhi",
        ],
        bestTime: "May 5 – June. 5 & October",
      },
      {
        name: "Best of Kumaon Tour",
        duration: "14 Nights / 13 Days",
        destinations: [
          "Delhi",
          "Nainital",
          "Mukteshwar",
          "Almora",
          "Binsar",
          "Kausani",
          "Bageshwar",
          "Champawat",
          "Bhimtal",
          "Delhi",
        ],
        bestTime: "Around The Year",
      },
      {
        name: "Haridwar – Rishikesh – Chamba – Mussoorie Tour",
        duration: "6 Nights / 7 Days",
        destinations: [
          "Delhi",
          "Haridwar",
          "Mussoorie",
          "Dhaunalti (or Dhaulatty etc.)",
          "Chamba",
          "Rishikesh",
          "Delhi",
        ],
        bestTime: "Around The Year",
      },
      {
        name: "Honeymoon in Mussoorie",
        duration: "6 Nights / 7 Days",
        destinations: [
          "Delhi",
          "Haridwar",
          "Mussoorie",
          "Dhaunalti",
          "Chamba",
          "Rishikesh",
          "Delhi",
        ],
        bestTime: "Around The Year",
      },
      {
        name: "Honeymoon in Nainital",
        duration: "6 Nights / 7 Days",
        destinations: ["Delhi", "Ramnagar", "Nainital", "Delhi"],
        bestTime: "Around The Year",
      },
      {
        name: "Golden Temple Tour",
        duration: "6 Nights / 7 Days",
        destinations: ["Delhi", "Amritsar", "Delhi"],
        bestTime: "Around The Year",
      },
      {
        name: "Golden Temple with Hemkund Sahib",
        duration: "16 Nights / 15 Days",
        destinations: [
          "Delhi",
          "Amritsar",
          "Haridwar",
          "Joshimath",
          "Govindghat",
          "Hemkund Sahib",
          "Joshimath",
          "Rishikesh",
          "Paonta Sahib",
          "Nanakmatha",
          "Manikaran Sahib",
          "Kullu",
          "Chandigarh",
          "Delhi",
        ],
        bestTime: "5 June to 5 October",
      },
    ],
    contact: {
      phone: ["+91-9412072802", "+91-9456332153", "01334-246280"],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
  },
  {
    id: "best-of-south-india",
    title: "Best Of South India Tours",
    description:
      "South India is the most famous region of India. It is a land of temples and one can also enjoy the honeymoon in its hill station like Ooty, Kodaikanal, and backwaters of Kerala are the best for new couple. India Holiday Home takes you the tour of South India and we hope that you will explore it nicely and let us know your experience.",
    duration: "", // no overall duration given
    // price: "", // no pricing given
    image: "https://images.unsplash.com/photo-1616849540275-0bc274e2a474?q=80&w=387&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D", // image(s) present but no single featured image URL specified
    rating: "", // no rating provided
    locations: [
      "Tamil Nadu",
      "Kerala",
      // also implicitly “South India” but specific states are not fully enumerated on page
    ],
    category: "Indiatour", // not explicitly labelled with e.g. “Cultural”, “Adventure”
    featured: false, // not marked as a featured tour
    highlights: [
      "Visit Bala Ji (Venkateswara) temple at Tirumala",
      "Temples, hill stations like Ooty, Kodaikanal; backwaters of Kerala are good for honeymooners",
      "Combination of pilgrimage and sightseeing in South India",
    ],
    itinerary: [
      {
        day: 1,
        title: "Delhi → Chennai",
        destinations: ["Delhi", "Chennai"],
        activities: [
          "Board flight/train from Delhi to Chennai",
          "Evening leisure or local sightseeing in Chennai",
        ],
      },
      {
        day: 2,
        title: "Chennai → Tirumala",
        destinations: ["Chennai", "Tirumala"],
        activities: [
          "Travel to Tirumala",
          "Evening Darshan at Tirumala Venkateswara Temple",
        ],
      },
      {
        day: 3,
        title: "Tirumala Darshan",
        destinations: ["Tirumala"],
        activities: [
          "Morning and evening Darshan at Tirumala Temple",
          "Explore local religious sites and surroundings",
        ],
      },
      {
        day: 4,
        title: "Tirumala → Chennai",
        destinations: ["Tirumala", "Chennai"],
        activities: [
          "Return to Chennai",
          "Leisure or optional sightseeing in Chennai",
        ],
      },
      {
        day: 5,
        title: "Chennai → Delhi",
        destinations: ["Chennai", "Delhi"],
        activities: [
          "Board flight/train back to Delhi",
          "End of Bala Ji Darshan Tour",
        ],
      },
    ],
    contact: {
      phone: ["+91-9412072802", "+91-9456332153", "01334-246280"], // plus another line has 8630090671 as a number
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
  },

  {
    id: "luxury-train-tours",
    title: "Luxury Train Tours",
    description:
      "Tours on luxury trains in India have become extremely popular. They’re an excellent way to explore the country without having to compromise on comfort. These luxury trains, which offer every indulgence imaginable right down to customized cutlery, inject glamor and romance into seeing some of India’s best tourist attractions. The luxury trains generally run from September to April each year. All meals, tours, and entrance fees to monuments and cultural sites are included in the price.",
    duration: "", // varies by train / itinerary; no single overall value given
    // price: "", // no fixed price for all, but some example rates are given
    image: "https://www.indianluxurytrains.com/wp-content/uploads/2011/11/Maharajas-Express-Train-1.jpg", // there are images on the page, but no single “featured” image URL extracted
    rating: "", // no rating info given
    locations: [
      "Rajasthan",
      "South India",
      "Maharashtra",
      "Goa",
      "Gujarat",
      "etc.", // various states depending on the train route
    ],
    category: "indiatour",
    featured: false, // not explicitly labelled as featured
    highlights: [
      "Palace on Wheels – 7-night itinerary through popular cities in Rajasthan and including the Taj Mahal",
      "Heritage Palace on Wheels – shorter & more affordable itineraries (Golden Triangle circuit; Desert Triangle Circuit)",
      "Golden Chariot – operates exclusively in South India; routes include Karnataka (Hampi, Badami, Mysore), also variants including Tamil Nadu, Pondicherry, Kerala",
      "Deccan Odyssey – multiple 7-night itineraries across Maharashtra, Goa, Gujarat, Rajasthan etc.",
      "Maharajas’ Express – opulent, multiple circuits; some focusing on North, some on South; starts from higher price points",
    ],
    itinerary: [
      {
        name: "Palace on Wheels",
        duration: "7 Nights",
        destinations: ["Rajasthan cities", "Taj Mahal", "…"], // generic; specifics not fully enumerated
        bestTime: "September to April",
      },
      {
        name: "Heritage Palace on Wheels – Golden Triangle Circuit",
        duration: "3 Nights / 4 Days",
        destinations: ["Delhi", "Jaipur", "Ranthambore", "Taj Mahal"],
        bestTime: "September to April",
      },
      {
        name: "Heritage Palace on Wheels – Desert Triangle Circuit",
        duration: "4 Nights / 5 Days",
        destinations: [
          "Shekhawati region",
          "Bikaner",
          "Jaipur",
          "Dholpur",
          "Taj Mahal",
        ],
        bestTime: "September to April",
      },
      {
        name: "Golden Chariot – Pride of South",
        duration: "7 Nights",
        destinations: [
          "Bangalore",
          "Mysore",
          "Hampi",
          "Badami",
          "Pattadakkal",
          "Belur",
          "Halebid",
          "Kabini",
          "Nagarhole",
          "Goa",
        ],
        bestTime: "September to April",
      },
      {
        name: "Golden Chariot – Southern Splendor",
        duration: "7 Nights",
        destinations: ["Bangalore", "Tamil Nadu", "Pondicherry", "Kerala", "…"],
        bestTime: "September to April",
      },
      {
        name: "Deccan Odyssey – multiple circuits",
        duration: "7 Nights",
        destinations: [
          "Maharashtra",
          "Goa",
          "Gujarat",
          "Rajasthan",
          "etc. depending on specific circuit",
        ],
        bestTime: "September to April",
      },
      {
        name: "Maharajas’ Express – various circuits",
        duration: "7 Nights",
        destinations: ["Golden Triangle", "South India circuits", "…"],
        bestTime: "September (for some southern itineraries) to April",
      },
    ],
    contact: {
      phone: ["+91-9412072802", "+91-9456332153", "01334-246280", "8630090671"],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
  },
  {
    id: "wild-tour-india",
    title: "Wild Tour Of India",
    description:
      "India Holiday Home introduce the Wild life packages for those who love to thrill and adventures in wild life. We invites you for a memorable time on wildlife tours in India. Choose any of our comprehensively designed tour in this category and explore the varied flora and fauna of various wildlife sanctuaries in India. We hope you will like it as one of the finest Wildlife destinations in the world, with its varied terrain, wetlands, jungles, deserts that provide excellent habitat for Tigers, Leopards, Lions, Rhinoceros, Elephants, Crocodiles, River Dolphins, Bears and variety of Deer, fabulous flowers and bird life. We have customized wildlife tours with different category, so you can see the Itinerary below and let us know to book any tour for you…",
    duration: "", // no common overall duration given
    // price: "", // no pricing information provided
    image: "https://plus.unsplash.com/premium_photo-1719922537914-526b2802c367?q=80&w=889&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D", // images present on page, but no specific featured image URL extracted
    rating: "", // not given
    locations: [
      "India", // generally everywhere in India where wildlife sanctuaries / national parks are
      // plus specific parks listed below
    ],
    category: "indiatour",
    featured: false,
    highlights: [
      "Visit multiple prominent national parks & sanctuaries such as Bandhavgarh, Ranthambore, Kaziranga, Kanha, Sundarbans, Manas, Bandipur & Nagarhole, etc.",
      "See diverse wildlife: tigers, leopards, lions, rhinoceros, elephants, crocodiles, river dolphins, bears, deer, etc.",
      "Explore varied terrain: jungles, wetlands, deserts, high altitude zones",
      "Customized wildlife tours (of different categories) to match interest in flora, fauna, birdlife",
    ],
    itinerary: [
      {
        day: 1,
        title: "Bandhavgarh & Ranthambore",
        description:
          "Start the wildlife adventure visiting central India’s famous parks.",
        activities: [
          "Morning safari at Bandhavgarh National Park",
          "Afternoon safari at Ranthambore National Park",
          "Overnight stay near Ranthambore",
        ],
      },
      {
        day: 2,
        title: "Kaziranga & Kanha",
        description: "Explore northeastern and central Indian wildlife.",
        activities: [
          "Early morning safari at Kaziranga National Park",
          "Evening safari at Kanha National Park",
          "Overnight stay near Kanha",
        ],
      },
      {
        day: 3,
        title: "Sunderbans & Manas",
        description:
          "Experience the unique ecosystems of mangroves and grasslands.",
        activities: [
          "Boat safari at Sunderbans National Park",
          "Visit Manas National Park for wildlife spotting",
          "Overnight stay near Manas",
        ],
      },
      {
        day: 4,
        title: "Bandipur & Sultanpur",
        description: "Discover southern and northern wildlife sanctuaries.",
        activities: [
          "Morning safari at Bandipur National Park",
          "Evening visit to Sultanpur National Park",
          "Overnight stay near Sultanpur",
        ],
      },
      {
        day: 5,
        title: "Raja Ji & Dudwha",
        description: "Explore northern India’s diverse flora and fauna.",
        activities: [
          "Safari at Raja Ji National Park",
          "Visit Dudwha National Park for tiger spotting",
          "Overnight stay near Dudwha",
        ],
      },
      {
        day: 6,
        title: "Bhalukpong & Simlipal",
        description: "Discover eastern and northeastern sanctuaries.",
        activities: [
          "Morning visit to Bhalukpong National Park",
          "Afternoon safari at Simlipal National Park",
          "Overnight stay nearby",
        ],
      },
      {
        day: 7,
        title: "Nandankanan & Gahirmatha",
        description: "Explore Odisha’s wildlife gems.",
        activities: [
          "Visit Nandankanan Zoo",
          "Visit Gahirmatha Turtle Sanctuary",
          "Overnight stay nearby",
        ],
      },
      {
        day: 8,
        title: "Namdapha & Velavadar",
        description: "Wildlife exploration in northeast and west India.",
        activities: [
          "Safari at Namdapha National Park",
          "Visit Velavadar Blackbuck Sanctuary",
          "Overnight stay nearby",
        ],
      },
      {
        day: 9,
        title: "Wild Ass & Dachigam",
        description: "Experience unique habitats of the desert and mountains.",
        activities: [
          "Safari at Wild Ass Sanctuary",
          "Visit Dachigam National Park",
          "Overnight stay nearby",
        ],
      },
      {
        day: 10,
        title: "Great Himalayan & Dibru Saikhowa",
        description:
          "Northern and northeastern high-altitude wildlife exploration.",
        activities: [
          "Visit The Great Himalayan National Park",
          "Safari at Dibru Saikhowa National Park",
          "Overnight stay nearby",
        ],
      },
      {
        day: 11,
        title: "Nameri, Him’s High Altitude & Nanda Devi",
        description:
          "Conclude the tour exploring Arunachal Pradesh and Uttarakhand sanctuaries.",
        activities: [
          "Visit Nameri National Park",
          "Explore Him’s High Altitude National Park",
          "Visit Nanda Devi National Park",
          "Return / overnight stay",
        ],
      },
    ],
    contact: {
      phone: [
        "+91-9412072802",
        "+91-9456 332153",
        "01334-246280",
        "8630090671",
      ],
      email: ["indiaholidayhome@gmail.com", "amitbajaj13@hotmail.com"], // assuming similar emails as other pages
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
    topDestinations: [
      "Amarnath Yatra",
      "Bala Ji Darshan",
      "Buddha Tours",
      "Golden Triangle",
      "Kailash",
      "Kumaon Tour",
    ],
  },
  {
    id: "cultural-tour-of-india",
    title: "Cultural Tour of India",
    description:
      "Heritage & Culture Tour India. Indian Culture endows the place with unique rituals, beliefs, attitudes, art forms, handicrafts and others. An enigmatic country like India has its root deep into its custom, culture, history and religion. Each state is different in this element creating a cluster of diversity that no other country can show. Tours focus on famous local festivals, epic rituals, traditions, customs, architecture, delicacies etc.",
    duration: "", // overall not fixed; different packages have different durations
    // price: "", // price not listed
    image: "https://images.unsplash.com/photo-1463592177119-bab2a00f3ccb?q=80&w=869&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D", // images present but no single featured image extracted
    rating: "", // no rating information given
    locations: [
      "Delhi",
      "Udaipur",
      "Jaipur",
      "Agra",
      "Varanasi",
      "South India (Chennai, Madurai, Kerala etc.)",
      "Rajasthan",
      "North India",
    ],
    category: "indiatour",
    featured: false,
    highlights: [
      "Experience the diversity of rituals, traditions, customs, architecture and cuisine across Indian states",
      "Attend or align with local festivals and heritage-events",
      "See influences of various cultures: British, Mughal, Tribal, Dutch, Tibetan etc.",
      "Explore major cultural and heritage centres: temples, palaces, ancient cities, traditional art forms",
    ],
    itinerary: [
      {
        day: 1,
        title: "Delhi Arrival",
        description: "Arrive in Delhi and check-in at the hotel.",
        activities: ["Airport pickup", "Hotel check-in", "Evening at leisure"],
      },
      {
        day: 2,
        title: "Delhi Sightseeing",
        description: "Explore key attractions in Delhi.",
        activities: [
          "Red Fort",
          "India Gate",
          "Qutub Minar",
          "Lotus Temple",
          "Local market visit",
        ],
      },
      {
        day: 3,
        title: "Delhi to Udaipur",
        description: "Travel to Udaipur by flight/train and check-in at hotel.",
        activities: [
          "Travel to Udaipur",
          "Hotel check-in",
          "Leisure at Fatehsagar Lake",
        ],
      },
      {
        day: 4,
        title: "Udaipur City Tour",
        description: "Sightseeing in Udaipur including palaces and lakes.",
        activities: [
          "City Palace",
          "Lake Pichola Boat Ride",
          "Jagdish Temple",
          "Saheliyon ki Bari",
        ],
      },
      {
        day: 5,
        title: "Udaipur to Jaipur",
        description: "Travel to Jaipur and check-in at hotel.",
        activities: ["Drive to Jaipur", "Hotel check-in", "Evening at leisure"],
      },
      {
        day: 6,
        title: "Jaipur Sightseeing",
        description: "Explore Jaipur including forts and markets.",
        activities: [
          "Amber Fort with elephant ride",
          "City Palace",
          "Jantar Mantar",
          "Hawa Mahal",
          "Local market visit",
        ],
      },
      {
        day: 7,
        title: "Jaipur to Agra",
        description: "Travel to Agra for Taj Mahal and Agra Fort visit.",
        activities: [
          "Drive to Agra",
          "Taj Mahal visit at sunrise",
          "Agra Fort exploration",
          "Check-in at hotel",
        ],
      },
      {
        day: 8,
        title: "Agra to Varanasi",
        description: "Travel to Varanasi via flight and explore the ghats.",
        activities: [
          "Flight to Varanasi",
          "Hotel check-in",
          "Evening Ganga Aarti at Dashashwamedh Ghat",
        ],
      },
      {
        day: 9,
        title: "Varanasi Sightseeing",
        description: "Explore temples, ghats, and local culture of Varanasi.",
        activities: [
          "Visit Kashi Vishwanath Temple",
          "Boat ride on Ganga",
          "Sarnath visit",
        ],
      },
      {
        day: 10,
        title: "Varanasi to Delhi",
        description: "Fly back to Delhi and departure.",
        activities: [
          "Flight to Delhi",
          "Transfer to airport or hotel for overnight stay",
        ],
      },
    ],
    contact: {
      phone: ["+91-9412072802", "+91-9456332153", "01334-246280", "8630090671"],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
  },
];
export const ReligiousTours = [
  {
    id: "jain-yatra",
    title: "Jain Yatra",
    description:
      "It is a beautiful and introductory trip of Jain Tours. We have 2 category of Packages for Jain Tours. Deluxe Category ::- In Deluxe Category, We are using Deluxe Hotels Camps. Advance booking is Must. Budget Category :- In Budget Category, We are using the Budget class Hotels Lodges Guest Houses. Advance booking is Must. The innumerable Jain Shrines in India spread the message of peace, non violence, love and enlightenment associated with the lives and activities of the Jain Tirthankars … India holds immense attraction for all those who desire to see the great Jain architectural splendour, and to know and understand the essence of this great religion …",
    duration: "9 Nights / 8 Days", // derived from itinerary: Day 1 to Day 9 (arrival day + 8 full days)
    // price: "", // not specified
    image: "https://www.livehindustan.com/lh-img/uploadimage/library/2022/04/12/16_9/16_9_6/famous_jain_temples_1649765728.jpg", // no single featured image URL explicitly given
    rating: "", // not provided
    locations: [
      "Delhi",
      "Udaipur",
      "Mount Abu",
      "Ranakpur",
      "Sravanabelagola",
      "Rajasthan",
      "Karnataka",
    ],
    category: "Spiritual",
    featured: false,
    highlights: [
      "Visit famous Jain temples at Falna, Dilwara (Mount Abu), Ranakpur, Sravanabelagola",
      "Two categories: Deluxe and Budget hotel accommodations",
      "Experience Jain architecture, art, and culture through various pilgrimage centres",
      "Tour covers Northern & Southern elements (Rajasthan + Karnataka)",
    ],
    itinerary: [
      {
        day: 1,
        title: "Arrival in Delhi",
        destinations: ["Delhi"],
        activities: [
          "Arrive in Delhi",
          "Transfer to hotel",
          "Evening at leisure",
        ],
      },
      {
        day: 2,
        title: "Delhi → Udaipur → Mount Abu",
        destinations: ["Delhi", "Udaipur", "Mount Abu"],
        activities: [
          "Travel to Udaipur",
          "Continue journey to Mount Abu",
          "Evening leisure in Mount Abu",
        ],
      },
      {
        day: 3,
        title: "Mount Abu Sightseeing",
        destinations: ["Mount Abu"],
        activities: [
          "Visit Dilwara Temples",
          "Explore Mount Abu local attractions",
        ],
      },
      {
        day: 4,
        title: "Mount Abu → Ranakpur",
        destinations: ["Mount Abu", "Ranakpur"],
        activities: [
          "Travel to Ranakpur",
          "Visit famous Jain Temples in Ranakpur",
        ],
      },
      {
        day: 5,
        title: "Ranakpur → Udaipur",
        destinations: ["Ranakpur", "Udaipur"],
        activities: ["Travel back to Udaipur", "Sightseeing in Udaipur"],
      },
      {
        day: 6,
        title: "Udaipur → Delhi",
        destinations: ["Udaipur", "Delhi"],
        activities: [
          "Return flight/train to Delhi",
          "Evening at leisure in Delhi",
        ],
      },
      {
        day: 7,
        title: "Delhi → Bangalore → Hassan → Shravanabelagola",
        destinations: ["Delhi", "Bangalore", "Hassan", "Shravanabelagola"],
        activities: [
          "Fly from Delhi to Bangalore",
          "Drive to Hassan",
          "Visit Shravanabelagola",
        ],
      },
      {
        day: 8,
        title: "Shravanabelagola → Bangalore → Delhi",
        destinations: ["Shravanabelagola", "Bangalore", "Delhi"],
        activities: [
          "Return to Bangalore",
          "Fly back to Delhi",
          "Departure from Delhi",
        ],
      },
    ],
    contact: {
      phone: [
        "+91-9412072802",
        "+91-9456-332153",
        "01334-246280",
        "8630090671",
      ],
      email: ["indiaholidayhome@gmail.com", "amitbajaj13@hotmail.com"], // as per site
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
  },
  {
    id: "sikh-tour",
    title: "Sikh Tour",
    description:
      "India is a country where one can find an amalgamation of different religions. These religions make the country’s culture more vibrant and diverse. One such religion spreading the message of peace is Sikhism. It is one of the youngest religions of the world and finds its roots in the Indian states like Punjab and Haryana. The shrines of Sikhism, Gurudwaras, display wonderful piece of architect and serenity. This 13 nights and 14 days itinerary takes the tourists to some of the famous Takhts of Sikhism. The tour lets the traveler explore different regions with religious significance in Sikhism such as Chandigarh, Ropar, Fategarh, Anandpur, Amritsar, Tarn Taran, Goindwal, and Patna. In addition, the itinerary also includes visiting magnificent tourist destinations such as Varanasi, Agra and Delhi.",
    duration: "14 Days / 13 Nights",
    // price: "", // Not mentioned on the page
    image: "https://images.unsplash.com/photo-1621377099913-ac1ec4848e52?q=80&w=435&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D", // Not a single featured image URL clearly specified
    rating: "", // No rating info on the page
    locations: [
      "Delhi",
      "Chandigarh",
      "Ropar",
      "Fategarh",
      "Anandpur",
      "Amritsar",
      "Tarn Taran",
      "Goindwal",
      "Patna",
      "Varanasi",
      "Agra",
    ],
    category: "Spiritual",
    featured: false, // Not labelled “featured”
    highlights: [
      "Visit the famous Sikh Takhts in Punjab, Haryana, Bihar including places like Amritsar, Anandpur etc.",
      "Explore historical Gurudwaras: Golden Temple, Goindwal, Tarn Taran, others in and around Punjab & Haryana",
      "Includes important non-Sikh religious / cultural destinations like Varanasi, Agra and Delhi as part of the pilgrimage tour",
      "Balanced itinerary mixing religious pilgrimage and sightseeing",
    ],
    itinerary: [
      {
        day: 1,
        title: "Arrival in Delhi",
        destinations: ["Delhi"],
        activities: [
          "Arrive in Delhi",
          "Transfer to hotel",
          "Evening at leisure",
        ],
      },
      {
        day: 2,
        title: "Delhi Sightseeing",
        destinations: ["Delhi"],
        activities: [
          "Visit India Gate, Qutub Minar, Lotus Temple",
          "Explore city markets",
          "Evening at leisure",
        ],
      },
      {
        day: 3,
        title: "Gurudwaras & Old Delhi Excursion",
        destinations: ["Delhi"],
        activities: [
          "Visit Gurudwara Bangla Sahib",
          "Explore Old Delhi: Chandni Chowk, Jama Masjid",
          "Evening free",
        ],
      },
      {
        day: 4,
        title: "Delhi → Chandigarh",
        destinations: ["Delhi", "Chandigarh"],
        activities: [
          "Drive/train to Chandigarh",
          "Check-in at hotel",
          "Evening at leisure",
        ],
      },
      {
        day: 5,
        title: "Chandigarh → Anandpur Sahib via Ropar & Fatehgarh Sahib",
        destinations: [
          "Chandigarh",
          "Ropar",
          "Fatehgarh Sahib",
          "Anandpur Sahib",
        ],
        activities: [
          "Visit historical sites en route",
          "Arrive at Anandpur Sahib",
          "Evening visit to Gurudwara",
        ],
      },
      {
        day: 6,
        title: "Amritsar Arrival",
        destinations: ["Amritsar"],
        activities: [
          "Drive/train to Amritsar",
          "Evening visit to Golden Temple",
        ],
      },
      {
        day: 7,
        title: "Full Day Golden Temple Tour",
        destinations: ["Amritsar"],
        activities: [
          "Visit Golden Temple complex",
          "Explore Jallianwala Bagh",
          "Evening at leisure",
        ],
      },
      {
        day: 8,
        title: "Excursion to Tarn Taran and Goindwal",
        destinations: ["Amritsar", "Tarn Taran", "Goindwal"],
        activities: [
          "Visit Gurudwaras in Tarn Taran and Goindwal",
          "Return to Amritsar in evening",
        ],
      },
      {
        day: 9,
        title: "Travel to Patna via Delhi",
        destinations: ["Amritsar", "Delhi", "Patna"],
        activities: [
          "Fly/train from Amritsar to Patna via Delhi",
          "Check-in at hotel in Patna",
        ],
      },
      {
        day: 10,
        title: "Visit Birthplace of Guru Gobind Singh",
        destinations: ["Patna"],
        activities: [
          "Visit Takht Sri Patna Sahib and other related sites",
          "Evening at leisure",
        ],
      },
      {
        day: 11,
        title: "Patna → Varanasi",
        destinations: ["Patna", "Varanasi"],
        activities: [
          "Travel to Varanasi",
          "Evening Ganga Aarti at Dashashwamedh Ghat",
        ],
      },
      {
        day: 12,
        title: "Varanasi → Agra",
        destinations: ["Varanasi", "Agra"],
        activities: [
          "Morning boat ride on the Ganges",
          "Sightseeing in Varanasi",
          "Travel to Agra by train/flight",
        ],
      },
      {
        day: 13,
        title: "Agra Sightseeing and Return to Delhi",
        destinations: ["Agra", "Delhi"],
        activities: [
          "Visit Taj Mahal, Agra Fort, Fatehpur Sikri (optional)",
          "Drive back to Delhi",
          "Evening at leisure",
        ],
      },
      {
        day: 14,
        title: "Departure from Delhi",
        destinations: ["Delhi"],
        activities: [
          "Check-out from hotel",
          "Transfer to airport/railway station",
          "Departure",
        ],
      },
    ],
    contact: {
      phone: [
        "+91-9412072802",
        "+91-9456-332153",
        "01334-246280",
        "8630090671",
      ],
      email: ["indiaholidayhome@gmail.com", "amitbajaj13@hotmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
  },
  {
    id: "hemkund-sahib-valley-flowers",
    title: "Hemkund Sahib Yatra & Valley of Flowers Trek",
    description:
      "Embark on a spiritual and scenic journey to Hemkund Sahib, a revered Sikh shrine at 4,320 meters, and the UNESCO-listed Valley of Flowers, known for its vibrant alpine flora. This trek offers a blend of spirituality, natural beauty, and adventure in the Garhwal Himalayas.",
    duration: "7 Days / 6 Nights",
    // price: "", // Price details are not explicitly mentioned on the page
    image:
      "https://images.unsplash.com/photo-1631377649998-58947055c21d?q=80&w=870&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D", // Example image URL
    rating: "", // No star rating or review count is available
    locations: [
      "Delhi",
      "Haridwar",
      "Govindghat",
      "Ghangharia",
      "Valley of Flowers",
      "Hemkund Sahib",
      "Rishikesh",
    ],
    category: "Spiritual",
    featured: false,
    highlights: [
      "Visit to Hemkund Sahib, a sacred Sikh shrine at 4,320 meters",
      "Trek through the UNESCO-listed Valley of Flowers",
      "Experience the rich biodiversity of alpine flora",
      "Blend of spiritual significance and natural beauty",
      "Guided trekking experience with experienced guides",
    ],
    itinerary: [
      {
        day: 1,
        title: "Delhi → Haridwar → Govindghat",
        destinations: ["Delhi", "Haridwar", "Govindghat"],
        activities: [
          "Travel from Delhi to Haridwar",
          "Proceed to Govindghat",
          "Overnight stay at Govindghat",
        ],
      },
      {
        day: 2,
        title: "Govindghat → Ghangharia",
        destinations: ["Govindghat", "Ghangharia"],
        activities: [
          "Trek/drive to Ghangharia",
          "Check-in at hotel/lodge",
          "Evening at leisure",
        ],
      },
      {
        day: 3,
        title: "Valley of Flowers Excursion",
        destinations: ["Ghangharia", "Valley of Flowers", "Ghangharia"],
        activities: [
          "Trek to Valley of Flowers",
          "Explore flora and scenic views",
          "Return to Ghangharia for overnight stay",
        ],
      },
      {
        day: 4,
        title: "Ghangharia → Hemkund Sahib → Ghangharia",
        destinations: ["Ghangharia", "Hemkund Sahib", "Ghangharia"],
        activities: [
          "Trek to Hemkund Sahib",
          "Perform darshan at Gurudwara",
          "Return to Ghangharia for overnight stay",
        ],
      },
      {
        day: 5,
        title: "Ghangharia → Govindghat → Haridwar",
        destinations: ["Ghangharia", "Govindghat", "Haridwar"],
        activities: [
          "Descend to Govindghat",
          "Drive to Haridwar",
          "Evening at leisure in Haridwar",
        ],
      },
      {
        day: 6,
        title: "Haridwar → Delhi",
        destinations: ["Haridwar", "Delhi"],
        activities: ["Travel back to Delhi", "End of tour"],
      },
    ],
    contact: {
      phone: ["+91‑9412072802", "+91‑9456332153", "8630090671"],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
    otherDetails: {
      availability: "May 25 – October 5",
      minAge: "12+",
      maxPeople: "30",
      region: "Uttarakhand",
    },
  },
  {
    id: "one-day-tour-example",
    title: "Bala Ji Darshan - One Day Tour",
    description:
      "A one-day pilgrimage tour from Chennai to Tirupati, including Quick Darshan tickets and meals.",
    duration: "1 Day",
    // price: "",
    image:
      "https://indiaholidayhome.com/wp-content/uploads/2024/02/tirupati-darshan-package.jpg",
    rating: "4.5/5",
    locations: ["Chennai", "Tirupati"],
    category: "Spiritual",
    featured: true,
    highlights: [
      "Quick Entry Darshan tickets included (₹300)",
      "AC Tempo Traveller transportation",
      "Breakfast and Lunch provided",
      "Pick-up and drop-off from designated points in Chennai",
      "Guided tour with optional tonsure ceremony",
    ],
    itinerary: [
      {
        day: 1,
        title: "Chennai → Tirumala → Chennai",
        destinations: ["Chennai", "Tirumala", "Chennai"],
        activities: [
          "05:30 AM: Pick-up from designated points in Chennai",
          "10:45 AM: Arrive at Tirumala",
          "11:45 AM: Quick Darshan at Tirupati Temple",
          "01:00 PM: Lunch at local restaurant / hotel",
          "02:00 PM: Free time / optional sightseeing nearby",
          "09:05 PM: Depart Tirumala and return to Chennai",
          "11:30 PM: Drop-off at starting points in Chennai",
        ],
      },
    ],
    contact: {
      phone: ["+91‑9412072802", "+91‑9456332153", "8630090671"],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
  },
  {
    id: "12-shiv-jyotirlinga",
    title: "12 Shiv Jyotirlinga Yatra",
    description:
      "A comprehensive 17-night, 18-day pilgrimage covering the 12 revered Jyotirlingas of Lord Shiva across India, including visits to sacred Char Dham destinations.",
    duration: "17 Nights / 18 Days",
    // price: "", // Price details are not explicitly mentioned on the page
    image:
      "https://indiaholidayhome.com/wp-content/uploads/2024/02/12-jyotirlinga-tour.jpg", // Example image URL
    rating: "", // No star rating or review count is available
    locations: [
      "Somnath (Gujarat)",
      "Dwarka (Gujarat)",
      "Ujjain (Madhya Pradesh)",
      "Omkareshwar (Madhya Pradesh)",
      "Kedarnath (Uttarakhand)",
      "Badrinath (Uttarakhand)",
      "Rameswaram (Tamil Nadu)",
      "Kashi Vishwanath (Uttar Pradesh)",
      "Grishneshwar (Maharashtra)",
      "Trimbakeshwar (Maharashtra)",
      "Bhimashankar (Maharashtra)",
      "Mallikarjuna (Andhra Pradesh)",
    ],
    category: "Spiritual",
    featured: true,
    highlights: [
      "Visit to all 12 sacred Jyotirlingas of Lord Shiva",
      "Darshan at the Char Dham destinations: Dwarka, Rameswaram, Kedarnath, and Badrinath",
      "Comprehensive 17-night, 18-day itinerary",
      "Guided pilgrimage ensuring spiritual enrichment",
      "Comfortable accommodations and transportation",
    ],
    itinerary: [],
    contact: {
      phone: ["+91‑9412072802", "+91‑9456332153", "8630090671"],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
  },
  {
    id: "kailash-mansarovar",
    title: "Kailash Mansarovar Yatra",
    description:
      "Embark on a sacred 16-day pilgrimage to Mount Kailash and Lake Mansarovar, traversing the serene landscapes of Tibet and Nepal. This journey offers spiritual enrichment and a deep connection with one of the most revered sites in Hinduism, Buddhism, Jainism, and Bon religion.",
    duration: "16 Days",
    // price: "₹1,75,000 per person",
    image:
      "https://indiaholidayhome.com/wp-content/uploads/2024/02/kailash-mansarovar.jpg",
    rating: "4.8/5",
    locations: [
      "Kathmandu",
      "Gyirong",
      "Saga",
      "Lake Mansarovar",
      "Darchen",
      "Dirapuk",
      "Zutulpuk",
    ],
    category: "Spiritual",
    featured: true,
    highlights: [
      "Visit to Mount Kailash and Lake Mansarovar",
      "Kora trek around Mount Kailash",
      "Stay at the serene Lake Mansarovar",
      "Cultural experiences in Tibet and Nepal",
      "Guided pilgrimage with experienced leaders",
    ],
    itinerary: [
      {
        day: 1,
        title: "Arrival in Kathmandu",
        destinations: ["Kathmandu"],
        activities: [
          "Arrive in Kathmandu",
          "Check-in at hotel",
          "Overnight stay in Kathmandu",
        ],
      },
      {
        day: 2,
        title: "Kathmandu Sightseeing",
        destinations: ["Kathmandu"],
        activities: [
          "Visit Pashupatinath Temple, Boudhanath Stupa, and other local attractions",
          "Overnight stay in Kathmandu",
        ],
      },
      {
        day: 3,
        title: "Kathmandu to Rasuwa Gadi",
        destinations: ["Kathmandu", "Rasuwa Gadi"],
        activities: [
          "Drive to Rasuwa Gadi, the Nepal-China border",
          "Overnight stay at Rasuwa Gadi",
        ],
      },
      {
        day: 4,
        title: "Rasuwa Gadi to Gyirong",
        destinations: ["Rasuwa Gadi", "Gyirong"],
        activities: [
          "Cross the border into Tibet",
          "Drive to Gyirong",
          "Overnight stay in Gyirong",
        ],
      },
      {
        day: 5,
        title: "Gyirong to Saga",
        destinations: ["Gyirong", "Saga"],
        activities: ["Drive to Saga", "Overnight stay in Saga"],
      },
      {
        day: 6,
        title: "Saga to Lake Mansarovar",
        destinations: ["Saga", "Lake Mansarovar"],
        activities: [
          "Drive to Lake Mansarovar",
          "Overnight stay near Lake Mansarovar",
        ],
      },
      {
        day: 7,
        title: "Lake Mansarovar to Darchen",
        destinations: ["Lake Mansarovar", "Darchen"],
        activities: [
          "Drive to Darchen, the base for the Kailash Kora",
          "Overnight stay in Darchen",
        ],
      },
      {
        day: 8,
        title: "Darchen to Dirapuk",
        destinations: ["Darchen", "Dirapuk"],
        activities: [
          "Begin the Kailash Kora trek",
          "Trek to Dirapuk",
          "Overnight stay in Dirapuk",
        ],
      },
      {
        day: 9,
        title: "Dirapuk to Zutulpuk",
        destinations: ["Dirapuk", "Zutulpuk"],
        activities: [
          "Continue the Kailash Kora trek",
          "Trek to Zutulpuk",
          "Overnight stay in Zutulpuk",
        ],
      },
      {
        day: 10,
        title: "Zutulpuk to Darchen",
        destinations: ["Zutulpuk", "Darchen"],
        activities: [
          "Complete the Kailash Kora trek",
          "Return to Darchen",
          "Overnight stay in Darchen",
        ],
      },
      {
        day: 11,
        title: "Darchen to Saga",
        destinations: ["Darchen", "Saga"],
        activities: ["Drive back to Saga", "Overnight stay in Saga"],
      },
      {
        day: 12,
        title: "Saga to Gyirong",
        destinations: ["Saga", "Gyirong"],
        activities: ["Drive to Gyirong", "Overnight stay in Gyirong"],
      },
      {
        day: 13,
        title: "Gyirong to Rasuwa Gadi",
        destinations: ["Gyirong", "Rasuwa Gadi"],
        activities: [
          "Drive back to Rasuwa Gadi",
          "Overnight stay at Rasuwa Gadi",
        ],
      },
      {
        day: 14,
        title: "Rasuwa Gadi to Kathmandu",
        destinations: ["Rasuwa Gadi", "Kathmandu"],
        activities: [
          "Cross the border back into Nepal",
          "Drive to Kathmandu",
          "Overnight stay in Kathmandu",
        ],
      },
      {
        day: 15,
        title: "Kathmandu Leisure Day",
        destinations: ["Kathmandu"],
        activities: [
          "Free day for shopping and leisure in Kathmandu",
          "Overnight stay in Kathmandu",
        ],
      },
      {
        day: 16,
        title: "Departure from Kathmandu",
        destinations: ["Kathmandu"],
        activities: [
          "Transfer to Tribhuvan International Airport for departure",
        ],
      },
    ],
    contact: {
      phone: ["+91‑9412072802", "+91‑9456332153", "8630090671"],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
  },
  {
    id: "amarnath-yatra",
    title: "Amarnath Yatra",
    description:
      "Embark on a sacred journey to the Amarnath Cave, home to the naturally occurring ice Shiva Lingam. This pilgrimage offers spiritual enrichment amidst the serene landscapes of the Himalayas.",
    duration: "5 Days / 4 Nights",
    // price: "₹25,000 per person",
    image:
      "https://indiaholidayhome.com/wp-content/uploads/2024/02/amarnath-yatra.jpg",
    rating: "4.8/5",
    locations: ["Srinagar", "Pahalgam", "Amarnath Cave"],
    category: "Spiritual",
    featured: true,
    highlights: [
      "Darshan of the naturally formed ice Shiva Lingam",
      "Trek through the scenic Pahalgam route",
      "Guided pilgrimage ensuring spiritual enrichment",
      "Comfortable accommodations and transportation",
    ],
    itinerary: [
      {
        day: 1,
        title: "Arrival in Srinagar",
        destinations: ["Srinagar"],
        activities: [
          "Arrive in Srinagar",
          "Check-in at hotel",
          "Overnight stay in Srinagar",
        ],
      },
      {
        day: 2,
        title: "Srinagar to Pahalgam",
        destinations: ["Srinagar", "Pahalgam"],
        activities: [
          "Drive from Srinagar to Pahalgam",
          "Check-in at hotel",
          "Overnight stay in Pahalgam",
        ],
      },
      {
        day: 3,
        title: "Pahalgam to Amarnath Cave",
        destinations: ["Pahalgam", "Amarnath Cave"],
        activities: [
          "Begin the trek from Pahalgam to Amarnath Cave",
          "Darshan of the ice Shiva Lingam",
          "Overnight stay near the cave",
        ],
      },
      {
        day: 4,
        title: "Return to Pahalgam",
        destinations: ["Amarnath Cave", "Pahalgam"],
        activities: [
          "Trek back to Pahalgam",
          "Check-in at hotel",
          "Overnight stay in Pahalgam",
        ],
      },
      {
        day: 5,
        title: "Departure from Pahalgam",
        destinations: ["Pahalgam", "Srinagar"],
        activities: ["Drive back to Srinagar", "Departure from Srinagar"],
      },
    ],
    contact: {
      phone: ["+91‑9412072802", "+91‑9456332153", "8630090671"],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
  },
  {
    id: "vaishno-devi-yatra",
    title: "Vaishno Devi Yatra",
    description:
      "Embark on a sacred journey to the Vaishno Devi Shrine, nestled in the Trikuta Mountains of Jammu & Kashmir. This pilgrimage offers spiritual enrichment amidst the serene landscapes of the Himalayas.",
    duration: "2 Days / 1 Night",
    // price: "₹3,999 per person",
    image:
      "https://indiaholidayhome.com/wp-content/uploads/2024/02/vaishno-devi-yatra.jpg",
    rating: "4.8/5",
    locations: ["Katra", "Vaishno Devi Shrine"],
    category: "Spiritual",
    featured: true,
    highlights: [
      "Darshan of the sacred Vaishno Devi Shrine",
      "Trek through the scenic Trikuta Mountains",
      "Comfortable accommodations and transportation",
    ],
    itinerary: [
      {
        day: 1,
        title: "Arrival in Katra",
        destinations: ["Katra"],
        activities: [
          "Arrive in Katra by train or bus",
          "Check-in at hotel",
          "Evening at leisure",
        ],
      },
      {
        day: 2,
        title: "Trek to Vaishno Devi Shrine",
        destinations: ["Katra", "Vaishno Devi Shrine"],
        activities: [
          "Early morning breakfast",
          "Begin trek to Vaishno Devi Shrine (12 km)",
          "Darshan at the shrine",
          "Return trek to Katra",
          "Departure from Katra",
        ],
      },
    ],
    contact: {
      phone: ["+91‑9412072802", "+91‑9456332153", "8630090671"],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
  },
  {
    id: "yoga-meditation-tour",
    title: "Yoga & Meditation Tour",
    description:
      "A transformative 9-day tour through the spiritual heartlands of India, offering immersive experiences in yoga and meditation amidst the serene landscapes of Haridwar, Uttarkashi, and Rishikesh.",
    duration: "8 Nights / 9 Days",
    // price: "₹25,000 per person",
    image:
      "https://indiaholidayhome.com/wp-content/uploads/2024/02/yoga-meditation-tour.jpg",
    rating: "4.7/5",
    locations: ["Haridwar", "Uttarkashi", "Rishikesh"],
    category: "Spiritual",
    featured: true,
    highlights: [
      "Daily yoga and meditation sessions",
      "Ganga Aarti at Haridwar and Rishikesh",
      "Nature walks and spiritual discussions",
      "White-water rafting in Rishikesh",
      "Comfortable accommodations and guided experiences",
    ],
    itinerary: [
      {
        day: 1,
        title: "Arrival in Haridwar",
        destinations: ["Haridwar"],
        activities: [
          "Arrive in Haridwar and check into hotel",
          "Evening participation in Ganga Aarti at Har Ki Pauri",
          "Overnight stay in Haridwar",
        ],
      },
      {
        day: 2,
        title: "Haridwar → Uttarkashi",
        destinations: ["Haridwar", "Uttarkashi"],
        activities: [
          "Drive to Uttarkashi (6-7 hours)",
          "Evening temple visits and local exploration",
          "Overnight stay in Uttarkashi",
        ],
      },
      {
        day: 3,
        title: "Yoga & Meditation in Uttarkashi",
        destinations: ["Uttarkashi"],
        activities: [
          "Morning yoga session",
          "Afternoon guided meditation",
          "Evening spiritual group discussion",
          "Overnight stay in Uttarkashi",
        ],
      },
      {
        day: 4,
        title: "Uttarkashi → Rishikesh",
        destinations: ["Uttarkashi", "Rishikesh"],
        activities: [
          "Drive to Rishikesh (5-6 hours)",
          "Evening exploration of local ashrams and markets",
          "Overnight stay in Rishikesh",
        ],
      },
      {
        day: 5,
        title: "Yoga & Meditation in Rishikesh",
        destinations: ["Rishikesh"],
        activities: [
          "Morning yoga at renowned ashram",
          "Afternoon guided meditation by Ganges",
          "Evening Ganga Aarti at Triveni Ghat",
          "Overnight stay in Rishikesh",
        ],
      },
      {
        day: 6,
        title: "River Rafting & Spiritual Exploration",
        destinations: ["Rishikesh"],
        activities: [
          "Morning white-water rafting on Ganges",
          "Afternoon visit to nearby temples",
          "Evening reflection and group sharing session",
          "Overnight stay in Rishikesh",
        ],
      },
      {
        day: 7,
        title: "Nature Walk & Meditation",
        destinations: ["Rishikesh"],
        activities: [
          "Morning nature walk in surrounding hills",
          "Afternoon silent meditation in nature",
          "Evening spiritual discussion",
          "Overnight stay in Rishikesh",
        ],
      },
      {
        day: 8,
        title: "Free Day in Rishikesh",
        destinations: ["Rishikesh"],
        activities: [
          "Explore Rishikesh at your own pace",
          "Optional activities: Beatles Ashram visit, shopping, extra yoga classes",
          "Overnight stay in Rishikesh",
        ],
      },
      {
        day: 9,
        title: "Departure from Rishikesh",
        destinations: ["Rishikesh", "Haridwar/Dehradun"],
        activities: [
          "Check out after breakfast",
          "Transfer to Haridwar or Dehradun for onward journey",
        ],
      },
    ],
    contact: {
      phone: ["+91‑9412072802", "+91‑9456332153", "8630090671"],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
  },
  {
    id: "source-of-ganges-tour",
    title: "Source of Ganges Tour",
    description:
      "Embark on a sacred journey to the origin of the Ganges River at Gaumukh, experiencing the spiritual and natural beauty of the Garhwal Himalayas.",
    duration: "6 Days / 5 Nights",
    // price: "₹18,000 per person",
    image:
      "https://indiaholidayhome.com/wp-content/uploads/2024/02/source-of-ganges-tour.jpg",
    rating: "4.7/5",
    locations: ["Delhi", "Haridwar", "Uttarkashi", "Gangotri", "Gaumukh"],
    category: "Spiritual",
    featured: true,
    highlights: [
      "Trek to the source of the Ganges at Gaumukh",
      "Visit to Gangotri Temple",
      "Scenic views of the Garhwal Himalayas",
      "Cultural experiences in Uttarkashi",
    ],
    itinerary: [
      {
        day: 1,
        title: "Delhi to Haridwar",
        destinations: ["Delhi", "Haridwar"],
        activities: [
          "Arrive at Delhi Railway Station",
          "Meet and assist, then drive to Haridwar (230 km / 6 hrs)",
          "Check-in at hotel",
          "Evening visit to Har-ki-Pauri for Ganga Aarti",
          "Overnight stay in Haridwar",
        ],
      },
      {
        day: 2,
        title: "Haridwar to Uttarkashi",
        destinations: ["Haridwar", "Uttarkashi"],
        activities: [
          "After breakfast, drive to Uttarkashi (190 km / 7 hrs)",
          "Check-in at hotel",
          "Evening at leisure or local exploration",
          "Overnight stay in Uttarkashi",
        ],
      },
      {
        day: 3,
        title: "Uttarkashi to Gangotri",
        destinations: ["Uttarkashi", "Gangotri"],
        activities: [
          "Drive to Gangotri (100 km / 4 hrs)",
          "Visit Gangotri Temple",
          "Evening at leisure",
          "Overnight stay in Gangotri",
        ],
      },
      {
        day: 4,
        title: "Gangotri to Bhojwasa",
        destinations: ["Gangotri", "Bhojwasa"],
        activities: [
          "Begin trek to Bhojwasa (14 km)",
          "Set up camp or check-in at guesthouse",
          "Evening at leisure",
          "Overnight stay in Bhojwasa",
        ],
      },
      {
        day: 5,
        title: "Bhojwasa to Gaumukh and Return to Bhojwasa",
        destinations: ["Bhojwasa", "Gaumukh"],
        activities: [
          "Early morning trek to Gaumukh (4 km)",
          "Visit the source of the Ganges",
          "Return trek to Bhojwasa (4 km)",
          "Overnight stay in Bhojwasa",
        ],
      },
      {
        day: 6,
        title: "Bhojwasa to Gangotri and Return to Uttarkashi",
        destinations: ["Bhojwasa", "Gangotri", "Uttarkashi"],
        activities: [
          "Trek back to Gangotri (14 km)",
          "Drive back to Uttarkashi",
          "Overnight stay in Uttarkashi",
        ],
      },
    ],
    contact: {
      phone: ["+91‑9412072802", "+91‑9456332153", "8630090671"],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
  },
  {
    id: "hemkund-sahib-valley-of-flowers",
    title: "Hemkund Sahib Yatra & Valley of Flowers",
    description:
      "Embark on a spiritual and scenic journey to Hemkund Sahib, a revered Sikh shrine, and the UNESCO-listed Valley of Flowers, renowned for its vibrant alpine flora.",
    duration: "6 Days / 5 Nights",
    // price: "₹17,000 per person",
    image:
      "https://www.thevalleyofflowers.com/wp-content/uploads/2024/02/hemkund-sahib-valley-of-flowers.jpg",
    rating: "4.8/5",
    locations: [
      "Haridwar",
      "Govindghat",
      "Ghangaria",
      "Hemkund Sahib",
      "Valley of Flowers",
    ],
    category: "Spiritual",
    featured: true,
    highlights: [
      "Darshan at Hemkund Sahib",
      "Trek through the Valley of Flowers",
      "Breathtaking views of the Himalayan landscape",
      "Guided tours with experienced trekkers",
    ],
    itinerary: [
      {
        day: 1,
        title: "Haridwar to Govindghat",
        destinations: ["Haridwar", "Govindghat"],
        activities: [
          "Depart from Haridwar in the morning",
          "Drive to Govindghat (approximately 10-12 hours)",
          "Check-in at hotel",
          "Overnight stay in Govindghat",
        ],
      },
      {
        day: 2,
        title: "Govindghat to Ghangaria",
        destinations: ["Govindghat", "Poolna", "Ghangaria"],
        activities: [
          "Drive to Poolna (starting point of the trek)",
          "Trek to Ghangaria (9 km, 5-6 hours)",
          "Check-in at guesthouse",
          "Overnight stay in Ghangaria",
        ],
      },
      {
        day: 3,
        title: "Ghangaria to Valley of Flowers",
        destinations: ["Ghangaria", "Valley of Flowers"],
        activities: [
          "Trek to Valley of Flowers (3.7 km each way)",
          "Explore the valley's diverse flora",
          "Return trek to Ghangaria",
          "Overnight stay in Ghangaria",
        ],
      },
      {
        day: 4,
        title: "Ghangaria to Hemkund Sahib",
        destinations: ["Ghangaria", "Hemkund Sahib"],
        activities: [
          "Trek to Hemkund Sahib (6 km, 5-6 hours)",
          "Darshan at the shrine",
          "Return trek to Ghangaria",
          "Overnight stay in Ghangaria",
        ],
      },
      {
        day: 5,
        title: "Ghangaria to Govindghat",
        destinations: ["Ghangaria", "Poolna", "Govindghat"],
        activities: [
          "Trek back to Poolna",
          "Drive to Govindghat",
          "Overnight stay in Govindghat",
        ],
      },
      {
        day: 6,
        title: "Govindghat to Haridwar",
        destinations: ["Govindghat", "Haridwar"],
        activities: ["Drive back to Haridwar", "Arrive in Haridwar by evening"],
      },
    ],
    contact: {
      phone: ["+91-9412072802", "+91-9456332153", "+91-8630090671"],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
    otherDetails: {
      availability: "June to September",
      minAge: "15 years",
      maxAge: "60 years",
      dailyLimit: "50 trekkers",
      registrationStart: "March 1, 2025",
      registrationLink: "https://indiaholidayhome.com/reservation/",
    },
  },
];
export const AdveturousTours = [
  {
    id: "skiing-tour",
    title: "Skiing Tour in Auli",
    description:
      "Embark on an exhilarating 8-day skiing adventure in Auli, Uttarakhand, experiencing the thrill of skiing on the pristine slopes amidst the majestic Himalayan backdrop.",
    duration: "8 Days / 7 Nights",
    // price: "₹25,000 per person",
    image:
      "https://indiaholidayhome.com/wp-content/uploads/2024/02/auli-skiing.jpg",
    rating: "4.7/5",
    locations: ["Auli", "Delhi", "Haridwar", "Joshimath", "Rishikesh"],
    category: "Adventure",
    featured: true,
    highlights: [
      "Skiing on the pristine slopes of Auli",
      "Stunning views of Nanda Devi and Trishul peaks",
      "Visit to major attractions in Delhi, Haridwar, and Rishikesh",
      "Guided skiing sessions for all skill levels",
      "Cultural experiences in the Himalayan region",
    ],
    itinerary: [
      {
        day: 1,
        title: "Arrival in Delhi",
        destinations: ["Delhi"],
        description: "Arrive in Delhi and settle in.",
        activities: [
          "Arrive in Delhi",
          "Transfer to hotel",
          "Evening at leisure",
          "Overnight stay in Delhi",
        ],
      },
      {
        day: 2,
        title: "Delhi to Haridwar",
        destinations: ["Delhi", "Haridwar"],
        description: "Travel to Haridwar and experience Ganga Aarti.",
        activities: [
          "Drive to Haridwar",
          "Check-in at hotel",
          "Visit Har-ki-Pauri for Ganga Aarti",
          "Overnight stay in Haridwar",
        ],
      },
      {
        day: 3,
        title: "Haridwar to Joshimath",
        destinations: ["Haridwar", "Joshimath"],
        description: "Drive to Joshimath and relax.",
        activities: [
          "Drive to Joshimath",
          "Check-in at hotel",
          "Evening at leisure",
          "Overnight stay in Joshimath",
        ],
      },
      {
        day: 4,
        title: "Joshimath to Auli",
        destinations: ["Joshimath", "Auli"],
        description: "Reach Auli and prepare for skiing.",
        activities: [
          "Drive to Auli",
          "Check-in at ski resort",
          "Introduction to skiing equipment and safety briefing",
          "Overnight stay in Auli",
        ],
      },
      {
        day: 5,
        title: "Skiing in Auli",
        destinations: ["Auli"],
        description: "Full day skiing sessions.",
        activities: [
          "Morning skiing session",
          "Lunch at resort",
          "Afternoon skiing session",
          "Evening at leisure",
          "Overnight stay in Auli",
        ],
      },
      {
        day: 6,
        title: "Skiing in Auli",
        destinations: ["Auli"],
        description: "Second day of skiing.",
        activities: [
          "Morning skiing session",
          "Lunch at resort",
          "Afternoon skiing session",
          "Evening at leisure",
          "Overnight stay in Auli",
        ],
      },
      {
        day: 7,
        title: "Auli to Rishikesh",
        destinations: ["Auli", "Rishikesh"],
        description: "Drive to Rishikesh and enjoy evening rituals.",
        activities: [
          "Drive to Rishikesh",
          "Check-in at hotel",
          "Visit Triveni Ghat for Ganga Aarti",
          "Overnight stay in Rishikesh",
        ],
      },
      {
        day: 8,
        title: "Rishikesh to Delhi",
        destinations: ["Rishikesh", "Delhi"],
        description: "Return to Delhi and conclude tour.",
        activities: [
          "Drive back to Delhi",
          "Drop at airport or railway station for onward journey",
        ],
      },
    ],
    contact: {
      phone: ["+91-9412072802", "+91-9456332153", "+91-8630090671"],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
    otherDetails: {
      availability: "December to March",
      minAge: "10 years",
      maxAge: "60 years",
      dailyLimit: "50 participants",
      registrationStart: "November 1, 2025",
      registrationLink: "https://indiaholidayhome.com/reservation/",
    },
  },
  {
    id: "rafting-rishikesh",
    title: "River Rafting in Rishikesh",
    description:
      "Experience the thrill of white-water rafting on the Ganges in Rishikesh, Uttarakhand. Navigate through exhilarating rapids amidst the scenic beauty of the Himalayan foothills.",
    duration: "1 Day",
    // price: "₹500–₹1,500 per person",
    image:
      "https://www.indianholiday.com/uttarakhand/rishikesh/river-rafting-in-rishikesh.html",
    rating: "4.5/5",
    locations: ["Rishikesh, Uttarakhand"],
    category: "Adventure",
    featured: true,
    highlights: [
      "Thrilling white-water rafting on the Ganges",
      "Experienced and certified instructors",
      "Safety gear provided",
      "Stunning views of the Himalayan foothills",
      "Suitable for beginners and experienced rafters",
    ],
    itinerary: [
      {
        day: 1,
        title: "Rafting Adventure in Rishikesh",
        destinations: ["Rishikesh"],
        description: "Single-day rafting experience.",
        activities: [
          "Arrive at the rafting base camp in Rishikesh",
          "Safety briefing and distribution of gear",
          "Start rafting session on the Ganges River",
          "Navigate through various rapids",
          "Return to base camp and conclude the session",
        ],
      },
    ],
    contact: {
      phone: ["+91-9412072802", "+91-9456332153", "+91-8630090671"],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
    otherDetails: {
      availability: "September to June",
      minAge: "12 years",
      maxAge: "60 years",
      dailyLimit: "Varies based on group size",
      registrationStart: "Advance booking recommended",
      registrationLink: "https://indiaholidayhome.com/reservation/",
    },
  },
  {
    id: "trekking-tours",
    title: "Trekking Tours in the Indian Himalayas",
    description:
      "Embark on exhilarating trekking adventures across the Indian Himalayas, exploring diverse terrains, spiritual sites, and breathtaking landscapes. Suitable for beginners and seasoned trekkers.",
    duration: "Varies by trek",
    // price: "Varies by trek",
    image:
      "https://indiaholidayhome.com/wp-content/uploads/2024/02/trekking.jpg",
    rating: "4.7/5",
    locations: [
      "Uttarakhand",
      "Himachal Pradesh",
      "Sikkim",
      "Arunachal Pradesh",
    ],
    category: "Adventure",
    featured: true,
    highlights: [
      "Explore renowned treks like Roopkund, Nanda Devi Base Camp, Dodital, and Valley of Flowers",
      "Experience spiritual journeys to Hemkund Sahib and the Sources of the Ganges",
      "Discover lesser-known trails such as Malari-Milam Trek",
      "Trekking options in Jammu & Kashmir, Himachal Pradesh, and Sikkim",
    ],
    itinerary: [
      {
        day: 1,
        title: "Arrival and Orientation",
        destinations: ["Base Camp"],
        description: "Arrive at base camp and settle in.",
        activities: [
          "Arrive at the designated base camp",
          "Safety briefing and distribution of trekking gear",
          "Overnight stay in base camp",
        ],
      },
      {
        day: 2,
        title: "Start of Trek",
        destinations: ["Base Camp", "First Trek Stop"],
        description: "Begin guided trek.",
        activities: [
          "Begin the trek with a guided hike through scenic landscapes",
          "Set up camp at the first trekking stop",
          "Evening campfire and group interaction",
          "Overnight stay in tents",
        ],
      },
      {
        day: 3,
        title: "Trekking and Exploration",
        destinations: ["First Trek Stop", "Next Trek Stop"],
        description: "Continue trekking.",
        activities: [
          "Continue trekking through varied terrains",
          "Visit local attractions or spiritual sites along the route",
          "Set up camp at the next stop",
          "Overnight stay in tents",
        ],
      },
      {
        day: 4,
        title: "Summit Day or Exploration",
        destinations: ["Next Trek Stop", "Base Camp"],
        description: "Summit or rest day.",
        activities: [
          "For summit treks: early morning ascent to the peak",
          "For other treks: exploration of nearby areas or rest day",
          "Return to base camp in the evening",
          "Overnight stay in base camp",
        ],
      },
      {
        day: 5,
        title: "Departure",
        destinations: ["Base Camp", "Starting Point"],
        description: "End of trekking tour.",
        activities: [
          "Pack up and depart from the base camp",
          "Return journey to the starting point",
        ],
      },
    ],
    contact: {
      phone: ["+91-9412072802", "+91-9456332153", "+91-8630090671"],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
    otherDetails: {
      availability: "May to October",
      minAge: "15 years",
      maxAge: "60 years",
      dailyLimit: "Varies by trek",
      registrationStart: "Advance booking recommended",
      registrationLink: "https://indiaholidayhome.com/reservation/",
    },
  },
  {
    id: "kayaking-rishikesh",
    title: "Kayaking Adventures in Rishikesh",
    description:
      "Experience the thrill of white-water kayaking on the Ganges River in Rishikesh. Suitable for all skill levels, with expert guidance and top-quality equipment.",
    duration: "Half-day to multi-day trips",
    // price: "₹1,000–₹4,000 per person",
    image:
      "https://indiaholidayhome.com/wp-content/uploads/2024/02/kayaking.jpg",
    rating: "4.6/5",
    locations: ["Rishikesh, Uttarakhand"],
    category: "Adventure",
    featured: true,
    highlights: [
      "Introductory sessions covering paddling strokes, turning techniques, and kayak balance",
      "Guided trips through rapids suitable for different skill levels",
      "Use of imported, high-quality kayaking gear",
      "Experienced instructors with international kayaking expertise",
      "Opportunities for both day trips and overnight camping adventures",
    ],
    itinerary: [
      {
        day: 1,
        title: "Arrival and Orientation",
        destinations: ["Rishikesh", "Kayaking Base Camp"],
        description: "Get familiar with kayaking basics.",
        activities: [
          "Arrive at the kayaking base camp in Rishikesh",
          "Safety briefing and distribution of kayaking gear",
          "Introduction to basic kayaking techniques",
          "Overnight stay at the base camp or nearby accommodation",
        ],
      },
      {
        day: 2,
        title: "Kayaking Adventure",
        destinations: ["Ganges River", "Rishikesh", "Kayaking Base Camp"],
        description: "Full-day kayaking experience.",
        activities: [
          "Morning kayaking session on the Ganges River",
          "Navigate through rapids tailored to your skill level",
          "Lunch break at a scenic spot along the river",
          "Afternoon kayaking session",
          "Return to base camp and conclude the trip",
          "Overnight stay at the base camp or nearby accommodation",
        ],
      },
    ],
    contact: {
      phone: ["+91-9412072802", "+91-9456332153", "+91-8630090671"],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
    otherDetails: {
      availability: "September to June",
      minAge: "12 years",
      maxAge: "60 years",
      dailyLimit: "Varies based on group size",
      registrationStart: "Advance booking recommended",
      registrationLink: "https://indiaholidayhome.com/reservation/",
    },
  },
  {
    id: "jeep-safari-india",
    title: "Jeep Safari Tour Packages India",
    description:
      "Jeep Safari tours arranged by wildlife sanctuaries are the best way to explore flora and fauna of the wildlife sanctuaries along with wildlife in a secured manner. Popular in India, these tours allow visitors to reach remote parts of the jungle quickly.",
    duration: "",
    // price: "",
    image: "",
    rating: "",
    locations: [
      "Dudhwa National Park",
      "Corbett National Park",
      "Gir National Park",
    ],
    category: "Wildlife",
    featured: true,
    highlights: [
      "Explore wildlife sanctuaries safely in jeeps",
      "Reach remote and inaccessible jungle areas",
      "Faster pace exploration of flora and fauna",
    ],
    itinerary: [],
    contact: {
      phone: ["+91-9412072802", "+91-9456332153", "+91-8630090671"],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
  },
  {
    id: "trans-himalayan-jeep-safari",
    title: "Trans Himalayan Jeep Safari",
    description:
      "Travel through high mountain passes with this thrilling 7-night/8-day Trans Himalayan Jeep Safari, covering desert valleys, lakes, monasteries, and scenic landscapes.",
    duration: "7 Nights / 8 Days",
    // price: "",
    image: "",
    rating: "",
    locations: [
      "Delhi",
      "Leh",
      "Nubra Valley",
      "Tsomoriri Lake",
      "Tsokar Lake",
      "Jispa",
      "Manali",
    ],
    category: "Wildlife",
    featured: true,
    highlights: [
      "Visit Phiyang & Spituk monasteries",
      "Camel safari at white sand dunes in Nubra",
      "Excursion to Tsokar Lake",
      "Explore Korzok village and surrounding landscapes",
    ],
    itinerary: [
      {
        day: 1,
        title: "Delhi – Leh",
        destinations: ["Delhi", "Leh"],
        activities: [
          "Arrival at Kushok Bakula Airport",
          "Hotel check-in",
          "Evening market stroll",
          "Overnight stay",
        ],
      },
      {
        day: 2,
        title: "Leh Sightseeing",
        destinations: [
          "Leh",
          "Phiyang Monastery",
          "Spituk Monastery",
          "Confluence of Indus & Zanskar",
          "Hall of Fame",
        ],
        activities: [
          "Visit Phiyang & Spituk Monasteries",
          "Picnic lunch",
          "Confluence of Indus & Zanskar",
          "Hall of Fame visit",
          "Overnight stay",
        ],
      },
      {
        day: 3,
        title: "Leh – Nubra Valley",
        destinations: ["Leh", "Nubra Valley"],
        activities: [
          "Travel to Nubra Valley",
          "Sightseeing and local activities",
          "Overnight stay",
        ],
      },
      {
        day: 4,
        title: "Nubra Valley – Leh",
        destinations: ["Nubra Valley", "Samstanling Monastery", "Leh"],
        activities: [
          "Explore Samstanling Monastery",
          "Return to Leh",
          "Evening shopping",
          "Overnight stay",
        ],
      },
      {
        day: 5,
        title: "Leh – Tsomoriri Lake",
        destinations: [
          "Leh",
          "Rushpu Valley",
          "Tsomoriri Lake",
          "Korzok Village",
        ],
        activities: [
          "Travel to Rushpu Valley",
          "Lunch en route",
          "Explore Tsomoriri Lake and Korzok village",
          "Overnight stay at camp",
        ],
      },
      {
        day: 6,
        title: "Tso Moriri Lake – Tsokar Lake",
        destinations: [
          "Tsomoriri Lake",
          "Tsokar Lake",
          "Puga",
          "Thukjey Village",
        ],
        activities: [
          "Drive to Tsokar Lake",
          "Visit geothermal hot springs near Puga",
          "Explore Thukjey village",
          "Overnight stay at camp",
        ],
      },
      {
        day: 7,
        title: "Tso-Kar Lake – Jispa",
        destinations: ["Tsokar Lake", "Sarchu Plains", "Jispa"],
        activities: [
          "Drive to Jispa via Sarchu plains",
          "Check into hotel",
          "Overnight stay",
        ],
      },
      {
        day: 8,
        title: "Jispa – Manali",
        destinations: ["Jispa", "Keylong", "Rohtang Pass", "Manali"],
        activities: [
          "Breakfast",
          "Drive to Manali via Keylong town",
          "Cross Rohtang Pass",
          "Check into hotel in Manali",
          "Overnight stay",
        ],
      },
    ],
    contact: {
      phone: ["+91-9412072802", "+91-9456332153", "+91-8630090671"],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
  },
  {
    id: "central-india-classic-tiger-tour",
    title: "Central India Classic Tiger Tour",
    description:
      "A 14-night/15-day wildlife and cultural tour across Madhya Pradesh and Mumbai covering temples, forts, caves, and tiger reserves.",
    duration: "14 Nights / 15 Days",
    // price: "",
    image: "",
    rating: "",
    locations: [
      "Delhi",
      "Varanasi",
      "Khajuraho",
      "Orchha",
      "Jhansi",
      "Bhopal",
      "Panchmarhi",
      "Jabalpur",
      "Kanha",
      "Mumbai",
    ],
    category: "Wildlife",
    featured: true,
    highlights: [
      "Excursion to Sarnath",
      "Visit Khajuraho Group of Monuments (UNESCO)",
      "Bhimbetka Rock Caves",
      "Sightseeing of Orchha and Bhopal attractions",
    ],
    itinerary: [
      {
        day: 1,
        title: "Delhi Arrival",
        destinations: ["Delhi"],
        activities: ["Airport transfer", "Hotel check-in", "Overnight stay"],
      },
      {
        day: 2,
        title: "Delhi Sightseeing",
        destinations: ["Delhi"],
        activities: [
          "Old & New Delhi tour: Jama Masjid, Red Fort, Lotus Temple, Qutub Minar, Humayun Tomb",
          "Overnight stay",
        ],
      },
      {
        day: 3,
        title: "Delhi – Varanasi",
        destinations: ["Delhi", "Varanasi", "Sarnath"],
        activities: [
          "Flight to Varanasi",
          "Visit Sarnath",
          "Evening Aarti",
          "Hotel overnight",
        ],
      },
      {
        day: 4,
        title: "Varanasi",
        destinations: ["Varanasi"],
        activities: [
          "Boat ride on Ganges",
          "Visit Kashi Vishwanath, Sankat Mochan Hanuman Temple",
          "Explore local shops",
          "Overnight stay",
        ],
      },
      {
        day: 5,
        title: "Varanasi – Khajuraho",
        destinations: ["Varanasi", "Khajuraho"],
        activities: [
          "Flight to Khajuraho",
          "Visit UNESCO Monuments",
          "Overnight stay",
        ],
      },
      {
        day: 6,
        title: "Khajuraho – Orchha",
        destinations: ["Khajuraho", "Orchha"],
        activities: [
          "Drive to Orchha",
          "Visit temples and palaces",
          "Overnight stay",
        ],
      },
      {
        day: 7,
        title: "Orchha – Jhansi – Bhopal",
        destinations: ["Orchha", "Jhansi", "Bhopal"],
        activities: [
          "Travel by train to Bhopal",
          "Sightseeing in Bhopal",
          "Overnight stay",
        ],
      },
      {
        day: 8,
        title: "Bhopal",
        destinations: ["Bhopal", "Sanchi"],
        activities: ["Visit Sanchi Stupa and Museum", "Overnight stay"],
      },
      {
        day: 9,
        title: "Bhopal – Bhimbetka – Bhojpur – Pachmarhi",
        destinations: ["Bhopal", "Bhimbetka", "Bhojpur", "Pachmarhi"],
        activities: [
          "Drive with stops at Bhimbetka and Bhojpur",
          "Overnight in Pachmarhi",
        ],
      },
      {
        day: 10,
        title: "Pachmarhi",
        destinations: ["Pachmarhi"],
        activities: [
          "Sightseeing Chauragarh Temple, Priyadarshini Point, Jatashankar Cave, Apsara Vihar, Bee Falls",
          "Overnight stay",
        ],
      },
      {
        day: 11,
        title: "Pachmarhi – Jabalpur",
        destinations: ["Pachmarhi", "Jabalpur"],
        activities: [
          "Drive to Jabalpur",
          "Boating at Marble Rocks",
          "Overnight stay",
        ],
      },
      {
        day: 12,
        title: "Jabalpur – Kanha",
        destinations: ["Jabalpur", "Kanha National Park"],
        activities: [
          "Transfer to Kanha National Park",
          "Hotel check-in",
          "Overnight stay",
        ],
      },
      {
        day: 13,
        title: "Kanha",
        destinations: ["Kanha National Park"],
        activities: ["Wildlife safari in Kanha", "Overnight stay"],
      },
      {
        day: 14,
        title: "Kanha – Nagpur – Mumbai",
        destinations: ["Kanha National Park", "Nagpur", "Mumbai"],
        activities: [
          "Drive to Nagpur, flight to Mumbai",
          "Hotel check-in",
          "Overnight stay",
        ],
      },
      {
        day: 15,
        title: "Mumbai",
        destinations: [
          "Mumbai",
          "Elephanta Caves",
          "Gateway of India",
          "Juhu Beach",
        ],
        activities: [
          "Excursion to Elephanta Caves, Gateway of India, Juhu Beach",
          "Airport transfer for onward journey",
        ],
      },
    ],
    contact: {
      phone: ["+91-9412072802", "+91-9456332153", "+91-8630090671"],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
  },
  {
    id: "land-of-the-tiger-tour",
    title: "Land of the Tiger Tour",
    description:
      "A 12-night/13-day tour exploring India’s tiger reserves, including Kanha and Bandhavgarh National Parks, along with cultural excursions to Agra and Delhi.",
    duration: "12 Nights / 13 Days",
    // price: "",
    image: "",
    rating: "",
    locations: ["Mumbai", "Nagpur", "Kanha", "Bandhavgarh", "Agra", "Delhi"],
    category: "Wildlife",
    featured: true,
    highlights: [
      "Sightseeing in Mumbai: Prince of Wales Museum, Hanging Gardens, Juhu Beach, Gateway of India, Marine Drive",
      "Jeep/Elephant Safari in Kanha and Bandhavgarh",
      "Guided tour of Agra and Delhi monuments including Taj Mahal and Red Fort",
      "Full-day game viewing in tiger reserves",
    ],
    itinerary: [
      {
        day: 1,
        title: "Mumbai Arrival",
        destinations: ["Mumbai"],
        activities: [
          "Airport transfer",
          "Hotel check-in",
          "Leisure day",
          "Overnight stay",
        ],
      },
      {
        day: 2,
        title: "Mumbai Sightseeing",
        destinations: ["Mumbai"],
        activities: [
          "Prince of Wales Museum, Hanging Gardens, Juhu Beach, Gateway of India, Marine Drive",
          "Overnight stay",
        ],
      },
      {
        day: 3,
        title: "Mumbai – Nagpur – Kanha",
        destinations: ["Mumbai", "Nagpur", "Kanha"],
        activities: [
          "Flight to Nagpur, drive to Kanha",
          "Check-in at wildlife resort",
          "Explore Kanha National Park",
          "Overnight stay",
        ],
      },
      {
        day: 4,
        title: "Kanha Safari",
        destinations: ["Kanha National Park"],
        activities: [
          "Jeep/Elephant Safari",
          "Visit Bamni Dadar, Kanha Museum, Raja Rani",
          "Overnight stay",
        ],
      },
      {
        day: 5,
        title: "Kanha Safari",
        destinations: ["Kanha National Park"],
        activities: ["Full-day game viewing in Kanha", "Overnight stay"],
      },
      {
        day: 6,
        title: "Kanha – Bandhavgarh",
        destinations: ["Kanha", "Bandhavgarh"],
        activities: [
          "Drive to Bandhavgarh",
          "Hotel check-in",
          "Overnight stay",
        ],
      },
      {
        day: 7,
        title: "Bandhavgarh Safari",
        destinations: ["Bandhavgarh National Park"],
        activities: [
          "Elephant Safari",
          "Trekking in natural trails",
          "Overnight stay",
        ],
      },
      {
        day: 8,
        title: "Bandhavgarh Safari",
        destinations: ["Bandhavgarh National Park"],
        activities: [
          "Jeep game drives",
          "Evening presentation on Bandhavgarh history",
          "Overnight stay",
        ],
      },
      {
        day: 9,
        title: "Bandhavgarh Sightseeing",
        destinations: ["Bandhavgarh National Park"],
        activities: [
          "Bandhavgarh Ancient Caves, Bandhavgarh Hill, Climbers Point",
          "Overnight stay",
        ],
      },
      {
        day: 10,
        title: "Bandhavgarh – Umaria – Agra",
        destinations: ["Bandhavgarh", "Umaria", "Agra"],
        activities: [
          "Elephant Safari in morning",
          "Transfer to Umaria, overnight train to Agra",
        ],
      },
      {
        day: 11,
        title: "Agra",
        destinations: ["Agra", "Fatehpur Sikri"],
        activities: [
          "Fatehpur Sikri visit",
          "Itmad-ud-Daula’s Tomb, Red Fort",
          "Overnight stay",
        ],
      },
      {
        day: 12,
        title: "Agra – Delhi",
        destinations: ["Agra", "Delhi"],
        activities: [
          "Taj Mahal visit",
          "Pearl Mosque, Itmad-ud-Daula tomb",
          "Train to Delhi, hotel check-in",
          "Overnight stay",
        ],
      },
      {
        day: 13,
        title: "Delhi Sightseeing & Departure",
        destinations: ["Delhi"],
        activities: [
          "Birla Mandir, Qutub Minar, Red Fort, India Gate, Jama Masjid, Humayun Tomb, President’s House",
          "Light & sound show at Red Fort",
          "Airport transfer",
        ],
      },
    ],
    contact: {
      phone: ["+91-9412072802", "+91-9456332153", "+91-8630090671"],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
  },
  {
    id: "haridwar-motor-biking-tour",
    title: "Haridwar Motor Biking Tour",
    description:
      "India Holiday Home offers a private motor biking tour in Haridwar, Uttarakhand, designed for solo travelers seeking a blend of adventure and spirituality.",
    duration: "4–6 hours",
    // price: "",
    image:
      "https://images.openai.com/thumbnails/url/GRYqt3icu1mSUVJSUGylr5-al1xUWVCSmqJbkpRnoJdeXJJYkpmsl5yfq5-Zm5ieWmxfaAuUsXL0S7F0Tw7JrSzNK81NT_PxLcnxzstJtPQPcHFKyypKCkpxSizMLSw39A_xcy42tnQNq_DIN84oT8zxSiwNNTb1D1QrBgBWTyrn",
    rating: "",
    locations: ["Haridwar, Uttarakhand"],
    category: "Adventure",
    featured: true,
    highlights: [
      "Explore Haridwar's historic temples, including Har Ki Pauri",
      "Experience vibrant local markets",
      "Interact with locals and observe daily life",
      "Taste authentic local dishes",
      "Witness the spiritual evening Ganga Aarti ceremony",
    ],
    itinerary: [
      {
        day: 1,
        title: "Haridwar Motor Biking Experience",
        activities: [
          "Pickup from Haridwar Railway Station or your hotel",
          "Ride through Haridwar visiting ancient temples",
          "Explore local markets",
          "Interact with locals and observe cultural life",
          "Enjoy local cuisine",
          "Witness evening Ganga Aarti at Har Ki Pauri",
          "Drop-off at pickup location",
        ],
      },
    ],
    contact: {
      phone: ["+91-9412072802"],
      email: ["indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
  },

  {
    id: "film-shooting-india",
    title: "Film Shooting Locations Tour",
    description:
      "Discover top film shooting locations in Uttarakhand, India, offering picturesque settings for filmmakers seeking both explored and unexplored sites.",
    duration: "Varies by destination",
    // price: "",
    image: "",
    rating: "",
    locations: [
      "Mussoorie",
      "Rishikesh",
      "Haridwar",
      "Dehradun",
      "Nainital",
      "Kausani",
      "Auli",
      "Almora",
    ],
    category: "Adventure",
    featured: false,
    highlights: [
      "Access to popular and unexplored filming locations",
      "Scenic landscapes and natural beauty",
      "Cultural and heritage sites for shooting",
      "Customized itineraries for filmmakers",
    ],
    itinerary: [
      {
        day: 1,
        destination: "Mussoorie",
        activities: [
          "Scouting locations for hill and valley shots",
          "Filming at Gun Hill, Camel's Back Road, and Mall Road",
          "Capture panoramic sunrise and sunset views",
        ],
      },
      {
        day: 2,
        destination: "Rishikesh",
        activities: [
          "Filming on the banks of the Ganga River",
          "Adventure sequences like rafting and bridges",
          "Scenic shots of Laxman Jhula and Ram Jhula",
        ],
      },
      {
        day: 3,
        destination: "Haridwar",
        activities: [
          "Evening Ganga Aarti filming at Har Ki Pauri",
          "Local market shots",
          "Cultural and spiritual sequences",
        ],
      },
      {
        day: 4,
        destination: "Dehradun",
        activities: [
          "Urban and natural shots in the city",
          "Forest sequences in nearby Rajaji National Park",
          "College and heritage building locations",
        ],
      },
      {
        day: 5,
        destination: "Nainital",
        activities: [
          "Lake and boat scenes at Naini Lake",
          "Mall Road and viewpoints for scenic filming",
          "Hilltop shots for panoramic views",
        ],
      },
      {
        day: 6,
        destination: "Kausani",
        activities: [
          "Capture Himalayan range views",
          "Filming tea gardens and local lifestyle",
          "Sunrise and sunset sequences",
        ],
      },
      {
        day: 7,
        destination: "Auli",
        activities: [
          "Filming skiing sequences in winter",
          "Panoramic shots of Nanda Devi peaks",
          "Adventure and snow activity sequences",
        ],
      },
      {
        day: 8,
        destination: "Almora",
        activities: [
          "Heritage architecture and local culture shots",
          "Himalayan valley filming",
          "Sunset sequences from panoramic points",
        ],
      },
    ],
    contact: {
      phone: ["+91-9412072802", "+91-9456332153", "+91-8630090671"],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
  },
  {
    id: "rock-climbing-india",
    title: "Rock Climbing Adventure Tours",
    description:
      "Experience thrilling rock climbing holidays across India, from beginner bouldering to advanced multi-pitch climbs, amidst scenic landscapes and historical sites.",
    duration: "Varies by package",
    // price: "",
    image: "",
    rating: "",
    locations: [
      "Delhi",
      "Haryana",
      "Uttarakhand",
      "Himachal Pradesh",
      "Rajasthan",
      "Maharashtra",
      "Madhya Pradesh",
      "West Bengal",
      "Karnataka",
      "Tamil Nadu",
    ],
    category: "Adventure",
    featured: false,
    highlights: [
      "Learn rock climbing skills in natural outdoor settings",
      "Opportunities for bouldering, face climbs, chimneys, and multi-pitch climbs",
      "Explore climbing areas with scenic, historical, and cultural significance",
      "Expert guidance with safety equipment provided",
      "Suitable for all levels, from beginners to advanced climbers",
    ],
    itinerary: [
      {
        day: 1,
        destination: "Delhi",
        activities: [
          "Ramjas Rocks: sandstone walls 20-60ft high, beginner to expert routes",
          "PBG Rocks: sandstone rocks 10-30ft high, forest land climbing",
          "Lado Sarai: sandstone formations with stacks and chimney climbs",
        ],
      },
      {
        day: 2,
        destination: "Around Delhi",
        activities: [
          "Dhauj, Haryana: quartzite rocks 10-40m high, over 250 routes",
          "Damdama, Haryana: climbs 20-120ft, protected by nuts and friends",
        ],
      },
      {
        day: 3,
        destination: "Uttarakhand",
        activities: [
          "Uttarkashi: Nehru Institute of Mountaineering, beginner bouldering",
          "Gangotri Gorge: granite walls 100m-1000m high, big-wall routes",
        ],
      },
      {
        day: 4,
        destination: "Himachal Pradesh",
        activities: [
          "Manikaran Spires: rock towers up to 15,000ft",
          "Manali and Kullu Valley: multiple climbing opportunities",
        ],
      },
      {
        day: 5,
        destination: "Rajasthan",
        activities: [
          "Mount Abu: volcanic rock formations, beginner-friendly climbs",
          "Brahma Kumari Ashram: training area for beginners",
          "Golden Horn Spire & Adhar Devi Slabs: climbs up to 150m",
        ],
      },
      {
        day: 6,
        destination: "West India",
        activities: [
          "Western Ghats around Mumbai: pinnacles and boulders for climbing",
          "Kanheri Caves, Sanjay Gandhi National Park, Borivali: beginner to advanced",
          "Parsik Hills, Mumbra & Manori Rocks, Malad: sea-front climbing and bouldering",
        ],
      },
      {
        day: 7,
        destination: "Central India",
        activities: [
          "Pachmarhi, Madhya Pradesh: multi-pitch climbs on sandstone cliffs",
        ],
      },
      {
        day: 8,
        destination: "East India",
        activities: [
          "Susunia Hills, West Bengal: granite and sandstone 1440ft high, beginner/intermediate",
          "Jai Chandi Hills: multi-pitch climbs, chimneys, overhangs",
          "Matha Buru, Ajodhya Hills: bouldering, traversing, advanced climbs",
          "Darjeeling: Tenzing and Gombu rocks at Himalayan Mountaineering Institute",
        ],
      },
      {
        day: 9,
        destination: "South India",
        activities: [
          "Hampi, Karnataka: granite boulders 4m-100m, 14 sq.km area",
          "Badami, Karnataka: sandstone cliffs and overhanging boulders",
          "Bangalore area: Ramanagram, Savandurga, Thuralli, Raogodlu for multi-pitch climbs",
          "Kambakkam, Tamil Nadu: camping, hiking, rock climbing",
        ],
      },
    ],
    contact: {
      phone: [
        "+91-9412072802",
        "+91-9456332153",
        "01334-246280",
        "+91-8630090671",
      ],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
  },
  {
    id: "para-gliding",
    title: "Para-Gliding in Uttarakhand",
    description:
      "Experience the thrill of paragliding and enjoy a bird’s eye view of the landscape. Fly a lightweight, free-flying, foot-launched glider aircraft with no rigid primary structure. The pilot sits in a harness suspended below a hollow fabric wing, offering an exhilarating experience.",
    duration: "Half-day to full-day trips",
    // price: "Varies by location and package",
    image:
      "https://indiaholidayhome.com/wp-content/uploads/2024/02/paragliding.jpg",
    rating: "4.6/5",
    locations: [
      "Rishikesh",
      "Mussoorie",
      "Pratap Nagar",
      "Dayara Bugyal",
      "Dhanaulti Ridge in Garhwal",
      "Naukutchiyatal in Nainital",
      "Pithoragarh",
      "Bedni Bugyal in Kumaon",
    ],
    category: "Adventure",
    featured: true,
    highlights: [
      "Soar high above the scenic landscapes of Uttarakhand",
      "Enjoy panoramic views of the Himalayan foothills",
      "Perfect for adventure enthusiasts, couples, and solo travelers",
      "All essential paragliding equipment provided",
      "Professional instructors and safety briefing included",
    ],
    itinerary: [
      {
        day: 1,
        title: "Para-Gliding Experience",
        description:
          "Exciting paragliding adventure with full safety and guidance.",
        activities: [
          "Arrive at the take-off site",
          "Safety briefing and gear distribution (Glider Harness, Helmet, Variometer, Reserve Parachute)",
          "Tandem flight with certified instructor",
          "Enjoy panoramic aerial views",
          "Landing and conclusion of the session",
        ],
      },
    ],
    contact: {
      phone: ["+91-9412072802", "+91-9456332153", "+91-8630090671"],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
    otherDetails: {
      bestTimeToVisit: "September to May",
      equipmentProvided: [
        "Glider Harness",
        "Helmet",
        "Variometer",
        "Reserve Parachute",
      ],
      suitableFor: ["Adventure Enthusiasts", "Couples", "Solo Travelers"],
      relatedActivities: [
        "Skiing",
        "Rafting",
        "Trekking",
        "Kayaking",
        "Jeep Safari",
        "Motor Biking",
        "Film Shooting",
        "Rock Climbing",
        "Elephant Safari",
        "Wild Life Safari",
      ],
      registrationLink: "https://indiaholidayhome.com/reservation/",
    },
  },
  {
    id: "elephant-safari",
    title: "Elephant Safari – India Holiday Home",
    description:
      "Elephant Safari in India is a unique and thrilling way to experience wildlife holidays. Ride on elephant back through dense forests, valleys, and rugged trails of Corbett National Park, spotting Royal Bengal Tigers, Great Single-horned Rhinoceros, leopards, hyenas, and crocodiles, accompanied by expert naturalists.",
    duration: "5 Nights / 6 Days",
    // price: "Varies by package",
    image:
      "https://indiaholidayhome.com/wp-content/uploads/2024/02/elephant-safari.jpg",
    rating: "4.7/5",
    locations: ["Corbett National Park, Uttarakhand"],
    category: "Wildlife",
    featured: true,
    highlights: [
      "Elephant safari in Corbett National Park",
      "Wildlife spotting: Tigers, Rhinos, Leopards, Hyenas, Crocodiles",
      "Guided tours with English-speaking naturalists",
      "Jeep safaris and scenic excursions along Ramganga river",
      "Stay in forest rest houses with British Raj style architecture",
      "Experience Indian & Western cuisine prepared by on-site chefs",
      "Opportunities for birdwatching and cultural interactions with local villagers",
    ],
    itinerary: [
      {
        day: 1,
        activities: [
          "Arrival at wildlife resort/camp near Corbett NP",
          "Check-in and lunch",
          "Jeep safari to Bijrani Range Forest",
          "Orientation talk and slide show on flora & fauna",
          "Dinner and overnight stay at resort/camp",
        ],
      },
      {
        day: 2,
        activities: [
          "Jeep ride to Durga Devi gate of Corbett Tiger Reserve",
          "Start elephant safari and trek",
          "Lunch by Ramganga river",
          "Elephant safari to Lohachaur Forest Rest House",
          "Discussion with naturalist and dinner",
        ],
      },
      {
        day: 3,
        activities: [
          "Full day elephant safari and wildlife observation",
          "Elephant bath in Mandal river",
          "Lunch prepared by on-site chef",
          "4WD Jeep transfer to Kanda Forest Rest House",
          "Evening discussion and dinner with naturalist",
        ],
      },
      {
        day: 4,
        activities: [
          "Jeep safari in Corbett NP, exploring Ramganga river and wildlife",
          "Lunch and relaxation at Dhikala Forest Lodge",
          "Elephant ride from Dhikala for wildlife viewing",
          "Evening tea, discussion, and dinner",
        ],
      },
      {
        day: 5,
        activities: [
          "Early morning elephant ride from Dhikala",
          "Breakfast",
          "Guided walk to watchtower for wildlife observation",
          "Jeep safari along lower jungle track of Ramganga river",
          "Return to resort/camp, bonfire and dinner",
        ],
      },
      {
        day: 6,
        activities: ["Breakfast", "Departure for onward destination"],
      },
    ],
    contact: {
      phone: ["+91-9412072802", "+91-9456332153", "01334-246280"],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
    otherDetails: {
      bestTimeToVisit: "November to March",
      registrationLink: "https://indiaholidayhome.com/reservation/",
    },
  },
  {
    id: "rajasthan-tour",
    title: "Rajasthan Tour – India Holiday Home",
    description:
      "Explore the royal heritage, deserts, and wildlife of Rajasthan. Visit iconic cities, national parks, cultural sites, and experience desert safaris and luxury tours across this vibrant state.",
    duration: "Varies by tour",
    // price: "Varies by package",
    image:
      "https://indiaholidayhome.com/wp-content/uploads/2024/02/rajasthan-tour.jpg",
    rating: "4.7/5",
    locations: [
      "Jaipur",
      "Bharatpur",
      "Sariska",
      "Alwar",
      "Shekhawati",
      "Ajmer",
      "Pushkar",
      "Ranthambhore",
      "Udaipur",
      "Mt Abu",
      "Jodhpur",
      "Jaisalmer",
      "Bikaner",
      "Delhi",
      "Agra",
    ],
    category: "indiatour",
    featured: true,
    highlights: [
      "Visit Jaipur, Bharatpur, Sariska National Park, Alwar, Shekhawati, Ajmer, Pushkar, Ranthambhore, Udaipur, Mt Abu, Jodhpur, Jaisalmer, Bikaner",
      "Desert safaris and wildlife tours",
      "Cultural experiences in historic cities and forts",
      "Luxury and adventure tours with expert guidance",
      "Photography opportunities and immersive experiences",
    ],
    itinerary: [],
    itineraries: [
      {
        tourName: "Desert Safari",
        duration: "14 Nights / 15 Days",
        destinations: [
          "Delhi",
          "Shekhawati",
          "Nawalgarh",
          "Bikaner",
          "Jaisalmer",
          "Jodhpur",
          "Udaipur",
          "Jaipur",
          "Delhi",
        ],
        bestTimeToVisit: "Around The Year",
      },
      {
        tourName: "Royal Rajasthan",
        duration: "16 Nights / 17 Days",
        destinations: [
          "Delhi",
          "Manesar",
          "Mandawa",
          "Bikaner",
          "Jaisalmer",
          "Manwar",
          "Jodhpur",
          "Rohetgarh",
          "Udaipur",
          "Pushkar",
          "Jaipur",
          "Agra",
          "Delhi",
        ],
        bestTimeToVisit: "Around The Year",
      },
      {
        tourName: "Magic of Rajasthan",
        duration: "19 Nights / 20 Days",
        destinations: [
          "Delhi",
          "Sikindra",
          "Agra",
          "Sariska",
          "Jaipur",
          "Mandawa",
          "Bikaner",
          "Jaisalmer",
          "Jodhpur",
          "Luni",
          "Ranakpur",
          "Udaipur",
          "Delhi",
        ],
        bestTimeToVisit: "1 October to end June",
      },
      {
        tourName: "Indian Tiger Wild Life in Rajasthan",
        duration: "7 Nights / 8 Days",
        destinations: [
          "Delhi",
          "Agra",
          "Bharatpur",
          "Ranthambhore National Park",
          "Sariska National Park",
          "Delhi",
        ],
        bestTimeToVisit: "1 October to end June",
      },
      {
        tourName: "Rajasthan Cultural Tour",
        duration: "15 Nights / 16 Days",
        destinations: [
          "Delhi",
          "Mandawa",
          "Bikaner",
          "Jaisalmer",
          "Jodhpur",
          "Kumbhalgarh",
          "Udaipur",
          "Kotah",
          "Bundi",
          "Ranthambore",
          "Jaipur",
          "Agra",
        ],
        bestTimeToVisit: "Around The Year",
      },
    ],
    contact: {
      phone: ["+91-9412072802", "+91-9456332153", "8630090671"],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
    otherDetails: {
      registrationLink: "https://indiaholidayhome.com/reservation/",
    },
  },
  {
    id: "wildlife-safari",
    title: "Wild Life Safari – India Holiday Home",
    description:
      "India’s diverse geography makes it a haven for wildlife lovers. With national parks, wildlife sanctuaries, and tiger reserves, visitors can experience a wide variety of flora and fauna including tigers, lions, rhinos, leopards, elephants, and numerous endemic species.",
    duration: "12 Days",
    // price: "Varies by package",
    image:
      "https://indiaholidayhome.com/wp-content/uploads/2024/02/wildlife-safari.jpg",
    rating: "4.7/5",
    locations: [
      "Ranthambore National Park, Rajasthan",
      "Jim Corbett National Park, Uttarakhand",
      "Bandhavgarh National Park, Madhya Pradesh",
      "Kanha National Park, Madhya Pradesh",
      "Kaziranga National Park, Assam",
      "Sunderban National Park, West Bengal",
      "Gir National Park, Gujarat",
      "Pench National Park, Madhya Pradesh",
      "Satpura National Park, Madhya Pradesh",
      "Tadoba National Park, Maharashtra",
      "Bandipur National Park, Karnataka",
      "Nagarhole National Park, Karnataka",
    ],
    category: "Wildlife",
    featured: true,
    highlights: [
      "Sight wildlife across plains, deserts, mountains, rivers, valleys, tropical rainforests, and coastal mangroves",
      "Explore 89 national parks, 489 wildlife sanctuaries, and 40 tiger reserves",
      "See iconic species like Tigers, Asiatic Lions, One-horned Rhinoceros, and Malabar Whistling Thrush",
      "Guided safari tours and nature excursions",
      "Opportunities for wildlife photography and birdwatching",
      "Cultural experiences near the wildlife destinations",
    ],
    itinerary: [
      {
        day: 1,
        destination: "Ranthambore National Park, Rajasthan",
        activities: [
          "Jeep safari to spot Tigers and other wildlife",
          "Photography opportunities with Sloth Bears, Wild Boars, and Sambar",
          "Explore the park's historic forts and desert landscapes",
        ],
        bestTimeToVisit: "October to June (peak tiger sighting: March–May)",
      },
      {
        day: 2,
        destination: "Jim Corbett National Park, Uttarakhand",
        activities: [
          "Guided safari through the oldest national park in Asia",
          "Spot Tigers, Fishing Cats, Serow, Goral, Himalayan Tahr",
          "Enjoy diverse flora and panoramic river views",
        ],
        bestTimeToVisit:
          "Mid November to June (peak tiger sighting: March–May)",
      },
      {
        day: 3,
        destination: "Bandhavgarh National Park, Madhya Pradesh",
        activities: [
          "Observe Royal Bengal Tigers in their natural habitat",
          "Experience rich biodiversity including other exotic fauna",
          "Guided forest walks and photography opportunities",
        ],
        bestTimeToVisit: "Mid October to June (peak tiger sighting: March–May)",
      },
      {
        day: 4,
        destination: "Kanha National Park, Madhya Pradesh",
        activities: [
          "Explore the inspiration behind The Jungle Book",
          "Spot Tigers, Black Panther, Elephants, Python, Bears",
          "Nature walks and birdwatching",
        ],
        bestTimeToVisit: "Mid October to June (peak tiger sighting: March–May)",
      },
      {
        day: 5,
        destination: "Kaziranga National Park, Assam",
        activities: [
          "Elephant and jeep safari to spot One-horned Rhinoceros",
          "Observe Elephants, Bears, Panthers, and diverse birdlife",
          "UNESCO World Heritage Site exploration",
        ],
        bestTimeToVisit: "November to April",
      },
      {
        day: 6,
        destination: "Sunderban National Park, West Bengal",
        activities: [
          "Boat safari through the Mangrove forests",
          "Spot Tigers in their amphibious habitat",
          "Birdwatching and river cruise experience",
        ],
        bestTimeToVisit: "October to April",
      },
      {
        day: 7,
        destination: "Gir National Park, Gujarat",
        activities: [
          "Jeep safari to spot Asiatic Lions",
          "Observe 32 species of mammals, 300 birds, 26 reptiles",
          "Guided nature walks and photography",
        ],
        bestTimeToVisit: "October to May",
      },
      {
        day: 8,
        destination: "Pench National Park, Madhya Pradesh",
        activities: [
          "Spot Tigers, Leopards, Wild Boar, Nilgai, Crimson-breasted Barbet",
          "Jeep safari and birdwatching",
          "Explore forests and streams",
        ],
        bestTimeToVisit: "Mid October to June (peak tiger sighting: March–May)",
      },
      {
        day: 9,
        destination: "Satpura National Park, Madhya Pradesh",
        activities: [
          "Adventure trekking in rugged terrain",
          "Spot Spotted Deer, Leopards, Sloth Bear, Black Buck, Sambar, Antelopes",
          "Photography and forest exploration",
        ],
        bestTimeToVisit: "October to April",
      },
      {
        day: 10,
        destination: "Tadoba National Park, Maharashtra",
        activities: [
          "Jeep safari to spot Tigers, Barking Deer, Hyenas, Gaur, Leopards",
          "Explore lush deciduous forests and lake areas",
          "Birdwatching and wildlife photography",
        ],
        bestTimeToVisit: "October to June (peak tiger sighting: March–May)",
      },
      {
        day: 11,
        destination: "Bandipur National Park, Karnataka",
        activities: [
          "Safari to spot Tigers, Gaur, Sambhar, Chital, Wild Dogs, Jackal, Sloth Bear, Panther",
          "Birdwatching in the Western Ghats",
          "Explore biodiversity-rich forest reserve",
        ],
        bestTimeToVisit: "October to June",
      },
      {
        day: 12,
        destination: "Nagarhole National Park, Karnataka",
        activities: [
          "Safari to spot Tigers, Leopards, Wild Dogs, Sloth Bear, Elephants, Deer",
          "Observe 250 species of avifauna",
          "Explore hills, valleys, streams, and waterfalls",
        ],
        bestTimeToVisit: "September to May",
      },
    ],
    contact: {
      phone: ["+91-9412072802", "+91-9456332153", "01334-246280"],
      email: ["amitbajaj13@hotmail.com", "indiaholidayhome@gmail.com"],
      address:
        "Sharanya, Brahm Vihar, Phase – 2, Kankhal – 249408, Haridwar, Uttarakhand, India",
    },
    otherDetails: {
      registrationLink: "https://indiaholidayhome.com/reservation/",
    },
  },
];
export const allTours = [
  ...featuredTours,
  ...CharDhamYatra,
  ...Honeymoon,
  ...IndiaTour,
  ...ReligiousTours,
  ...AdveturousTours,

  {
    id: "himalayan-adventure",
    title: "Himalayan Adventure",
    description: "Trek through the magnificent Himalayan mountains and valleys",
    duration: "10 Days / 9 Nights",
    // price: 1299,
    image:
      "https://images.unsplash.com/photo-1602615576820-ea14cf3e476a?q=80&w=1200",
    rating: 4.8,
    locations: ["Manali", "Leh", "Nubra Valley", "Pangong Lake"],
    category: "Adventure",
    featured: false,
    highlights: [
      "Drive over the world's highest motorable roads",
      "Camp under the stars at Pangong Lake",
      "Experience the unique Ladakhi culture",
      "Visit ancient monasteries perched on hilltops",
      "Thrilling white water rafting on the Zanskar river",
    ],
    itinerary: [
      {
        day: 1,
        title: "Arrival in Manali",
        description:
          "Arrive and acclimatize in Manali. Briefing about the journey ahead.",
        activities: [
          "Arrival in Manali",
          "Trip briefing",
          "Equipment check",
          "Welcome dinner",
        ],
      },
      {
        day: 2,
        title: "Manali Exploration",
        description:
          "Visit Hadimba Temple, Vashisht Hot Springs, and prepare for the journey.",
        activities: [
          "Local sightseeing",
          "Adventure equipment shopping",
          "Acclimatization walks",
        ],
      },
      {
        day: 3,
        title: "Manali to Keylong",
        description:
          "Drive over Rohtang Pass to reach Keylong. Stunning landscapes along the way.",
        activities: [
          "Rohtang Pass crossing",
          "Photography stops",
          "First Himalayan night",
        ],
      },
      {
        day: 4,
        title: "Keylong to Leh",
        description:
          "Long drive through Baralacha La and Moore Plains to reach Leh.",
        activities: [
          "High mountain passes",
          "Suraj Tal lake visit",
          "Evening in Leh market",
        ],
      },
      {
        day: 5,
        title: "Leh Acclimatization",
        description:
          "Rest day in Leh with short visits to Shanti Stupa and Leh Palace.",
        activities: [
          "Leh Palace tour",
          "Shanti Stupa visit",
          "Local market exploration",
        ],
      },
      {
        day: 6,
        title: "Leh to Nubra Valley",
        description:
          "Cross Khardung La, the world's highest motorable pass, to reach Nubra Valley.",
        activities: [
          "Khardung La crossing",
          "Diskit Monastery",
          "Camel safari on sand dunes",
        ],
      },
      {
        day: 7,
        title: "Nubra Valley Exploration",
        description: "Visit Diskit Monastery, sand dunes, and hot springs.",
        activities: [
          "Buddhist monastery tour",
          "Sand dune adventure",
          "Panamik hot springs",
        ],
      },
      {
        day: 8,
        title: "Nubra to Pangong Lake",
        description:
          "Travel to the stunning Pangong Lake through Shyok river route.",
        activities: [
          "Scenic drive",
          "Pangong Lake arrival",
          "Lakeside camping",
        ],
      },
      {
        day: 9,
        title: "Pangong Lake to Leh",
        description:
          "Morning by the lake, then return to Leh via Chang La pass.",
        activities: [
          "Sunrise photography",
          "Chang La pass crossing",
          "Farewell dinner in Leh",
        ],
      },
      {
        day: 10,
        title: "Departure from Leh",
        description: "Transfer to Leh airport for departure.",
        activities: ["Souvenir shopping", "Airport transfer"],
      },
    ],
  },
  {
    id: "goa-beaches",
    title: "Goa Beach Getaway",
    description:
      "Relax on the stunning beaches of Goa with exciting water sports",
    duration: "5 Days / 4 Nights",
    // price: 549,
    image:
      "https://images.unsplash.com/photo-1476158085676-e67f57ed9ed7?q=80&w=1200",
    rating: 4.5,
    locations: ["North Goa", "South Goa"],
    category: "Beach",
    featured: false,
    highlights: [
      "Sunset cruise on the Arabian Sea",
      "Water sports at Baga Beach",
      "Visit to historic Portuguese-era churches",
      "Vibrant night markets and beach parties",
      "Fresh seafood dining experiences",
    ],
    itinerary: [
      {
        day: 1,
        title: "Arrival in Goa",
        description:
          "Arrive at Goa Airport. Transfer to beach resort. Evening leisure time at the beach.",
        activities: [
          "Airport pickup",
          "Resort check-in",
          "Beach relaxation",
          "Welcome cocktail",
        ],
      },
      {
        day: 2,
        title: "North Goa Beaches",
        description:
          "Explore North Goa beaches including Baga, Calangute, and Anjuna. Try water sports and visit the famous flea market.",
        activities: [
          "Beach hopping",
          "Water sports",
          "Anjuna flea market",
          "Beach shack dinner",
        ],
      },
      {
        day: 3,
        title: "Old Goa and Panjim",
        description:
          "Visit the historic churches of Old Goa and explore Panjim city. Evening sunset cruise on the Mandovi River.",
        activities: [
          "Old Goa churches tour",
          "Panjim city walk",
          "Sunset river cruise",
          "Goan cuisine dinner",
        ],
      },
      {
        day: 4,
        title: "South Goa Exploration",
        description:
          "Day trip to the more serene beaches of South Goa including Palolem and Colva. Visit to spice plantation on the way.",
        activities: [
          "South Goa beach visit",
          "Spice plantation tour",
          "Beach yoga session",
          "Seafood dinner",
        ],
      },
      {
        day: 5,
        title: "Departure",
        description:
          "Morning free for last-minute shopping or beach time. Transfer to airport for departure.",
        activities: ["Beach time", "Souvenir shopping", "Airport transfer"],
      },
    ],
  },
  {
    id: "northeast-explorer",
    title: "Northeast Explorer",
    description: "Discover the untouched beauty of India's northeastern states",
    duration: "12 Days / 11 Nights",
    // price: 1499,
    image:
      "https://images.unsplash.com/photo-1605649487212-47bdab064df7?q=80&w=1200",
    rating: 4.9,
    locations: ["Gangtok", "Darjeeling", "Kaziranga", "Shillong"],
    category: "adventure",
    featured: false,
    highlights: [
      "Jeep safari in Kaziranga National Park",
      "Tea plantation tour in Darjeeling",
      "View of Mt. Kanchenjunga at sunrise",
      "Living root bridges in Meghalaya",
      "Experience diverse tribal cultures and cuisines",
    ],
    itinerary: [
      {
        day: 1,
        title: "Arrival in Bagdogra",
        description:
          "Arrive at Bagdogra Airport. Drive to Darjeeling. Evening at leisure.",
        activities: [
          "Airport pickup",
          "Scenic drive to Darjeeling",
          "Hotel check-in",
          "Welcome dinner",
        ],
      },
      {
        day: 2,
        title: "Darjeeling Exploration",
        description:
          "Early morning visit to Tiger Hill for sunrise over Kanchenjunga. Visit tea plantations and the Himalayan Mountaineering Institute.",
        activities: [
          "Tiger Hill sunrise",
          "Tea estate visit",
          "Joy ride on Darjeeling Toy Train",
          "Local market exploration",
        ],
      },
      {
        day: 3,
        title: "Darjeeling to Gangtok",
        description: "Drive to Gangtok. Evening walk on MG Road.",
        activities: [
          "Scenic drive to Sikkim",
          "Border crossing formalities",
          "Evening cultural show",
        ],
      },
      {
        day: 4,
        title: "Gangtok Sightseeing",
        description:
          "Visit to Rumtek Monastery, Enchey Monastery, and other attractions around Gangtok.",
        activities: [
          "Monastery visits",
          "Cable car ride",
          "Traditional Sikkimese dinner",
        ],
      },
      {
        day: 5,
        title: "Excursion to Tsomgo Lake",
        description:
          "Day trip to the high-altitude Tsomgo Lake and Baba Mandir.",
        activities: [
          "High-altitude lake visit",
          "Yak ride",
          "Mountain photography",
        ],
      },
      {
        day: 6,
        title: "Gangtok to Guwahati",
        description:
          "Fly from Bagdogra to Guwahati. Evening visit to Brahmaputra River front.",
        activities: [
          "Airport transfer",
          "Flight to Guwahati",
          "River front evening walk",
        ],
      },
      {
        day: 7,
        title: "Guwahati to Kaziranga",
        description:
          "Drive to Kaziranga National Park. Evening cultural program at resort.",
        activities: [
          "Transfer to Kaziranga",
          "Lodge check-in",
          "Briefing about safari",
          "Cultural show",
        ],
      },
      {
        day: 8,
        title: "Kaziranga Safari",
        description:
          "Morning elephant safari and afternoon jeep safari in different ranges of the park to spot one-horned rhinos.",
        activities: [
          "Elephant safari",
          "Jeep safari",
          "Birdwatching",
          "Wildlife photography",
        ],
      },
      {
        day: 9,
        title: "Kaziranga to Shillong",
        description:
          "Drive to Shillong through scenic landscapes. Evening at leisure.",
        activities: [
          "Scenic drive",
          "Stop at Umiam Lake",
          "Shillong peak visit",
        ],
      },
      {
        day: 10,
        title: "Shillong Exploration",
        description:
          "Visit local attractions including Don Bosco Museum and Ward's Lake.",
        activities: [
          "Don Bosco Museum",
          "Golf course visit",
          "Local cuisine exploration",
        ],
      },
      {
        day: 11,
        title: "Excursion to Cherrapunji",
        description:
          "Day trip to Cherrapunji to see living root bridges and waterfalls.",
        activities: [
          "Root bridge trekking",
          "Waterfall visits",
          "Cave exploration",
        ],
      },
      {
        day: 12,
        title: "Departure",
        description: "Transfer to Guwahati airport for departure.",
        activities: ["Shillong to Guwahati transfer", "Airport drop-off"],
      },
    ],
  },
];

export const tourCategories = [
  {
    id: "cultural",
    name: "Cultural Tours",
    description:
      "Explore India's rich cultural heritage and historical monuments",
    image:
      "https://images.unsplash.com/photo-1605649487212-47bdab064df7?q=80&w=800",
    count: 12,
  },
  {
    id: "indiatour",
    name: "India Tour",
    description:
      "Experience India's diverse wildlife in national parks and sanctuaries",
    image:
      "https://images.unsplash.com/photo-1564989549226-7eb5f06e40ca?q=80&w=800",
    count: 6,
  },
  {
    id: "spiritual",
    name: "Spiritual Tours",
    description:
      "Connect with your inner self through India's spiritual destinations",
    image:
      "https://images.unsplash.com/photo-1561361058-c24cecde1229?q=80&w=800",
    count: 8,
  },
  {
    id: "adventure",
    name: "Adventure Tours",
    description:
      "Get your adrenaline pumping with exciting adventure activities",
    image:
      "https://images.unsplash.com/photo-1544650039-3ef40c32a685?q=80&w=800",
    count: 9,
  },
  {
    id: "wildlife",
    name: "Wildlife Tours",
    description:
      "Experience India's diverse wildlife in national parks and sanctuaries",
    image:
      "https://images.unsplash.com/photo-1564989549226-7eb5f06e40ca?q=80&w=800",
    count: 6,
  },
  {
    id: "honeymoon",
    name: "Honeymoon Tours",
    description:
      "Experience India's diverse wildlife in national parks and sanctuaries",
    image:
      "https://images.unsplash.com/photo-1564989549226-7eb5f06e40ca?q=80&w=800",
    count: 6,
  },
  {
    id: "beach",
    name: "Beach",
    description:
      "Explore India's rich cultural heritage and historical monuments",
    image:
      "https://images.unsplash.com/photo-1605649487212-47bdab064df7?q=80&w=800",
    count: 12,
  },
];

export const carRental = {
  categories: [
    {
      id: "economy",
      name: "Economy",
      image:
        "https://images.unsplash.com/photo-1549317661-bd32c8ce0db2?q=80&w=800",
      startingPrice: 30,
    },
    {
      id: "standard",
      name: "Standard",
      image:
        "https://images.unsplash.com/photo-1533473359331-0135ef1b58bf?q=80&w=800",
      startingPrice: 45,
    },
    {
      id: "premium",
      name: "Premium",
      image:
        "https://images.unsplash.com/photo-1494976388531-d1058494cdd8?q=80&w=800",
      startingPrice: 70,
    },
    {
      id: "suv",
      name: "SUV",
      image:
        "https://images.unsplash.com/photo-1503736334956-4c8f8e92946d?q=80&w=800",
      startingPrice: 85,
    },
  ],
  services: [
    "Airport transfers",
    "City tours",
    "Intercity travel",
    "Custom itineraries",
    "Multi-day rentals",
    "Chauffeur services",
  ],
};
